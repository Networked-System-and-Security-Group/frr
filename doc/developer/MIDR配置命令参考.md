...
路径还必须为 valid、selected/multipath 且 non-stale；如配置了 route-map，还必须被 route-map permit。配置写入 running-config，非默认值才会显示；变化会重扫当前 Unicast RIB 并原子重建 Contributor Table。`ZEBRA_ROUTE_MIDR` 作为 MIDR 路由安装的独立 FRR route type 保留，但不参与本准入条件的硬过滤。
