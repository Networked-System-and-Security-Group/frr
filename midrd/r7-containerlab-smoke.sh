...
command -v docker >/dev/null 2>&1 || {
	echo 'docker is required for the R7 smoke' >&2
	exit 2
}
...
EOF
	...
	for i in $(seq 1 160); do
		...
		for n in a b c; do
			...
			if ! docker exec "$container" grep -q \
				'midrd node=.* final-objects=' /tmp/midrd.log 2>/dev/null; then
				all=0
				break
			fi
		done
	...
	for n in a b c; do
		...
		grep -Eq 'shutdown=(COMPLETE|DEGRADED)' "$run/$n.log"
	done
	...
	if [[ "$expected_routes" == 0 ]]; then
		...
		grep -q "route originator=$node_c metric=12 reachable=1" "$run/a.log"
		grep -q "route originator=$node_a metric=12 reachable=1" "$run/c.log"
	fi
...
run_cluster ipv4-withdraw 4 31101 1 3 0 \
