...
#include "midrd/group1/midr_pm_net.h"

DEFINE_MTYPE_STATIC(MIDR_G1, MIDR_PROBE_CTX, "MIDR probe context");
...
static struct midr_probe_ctx *pm_ctx_find(struct midr_nds *mi,
		       ...
		       enum midr_link_status status)
{
	...
	midr_nds_on_link_update(g1, &ctx->target, status,
...
{
	struct midr_nds *mi = ctx->g1->midr_nds_info;
	...
	if (mi->pm_sock < 0)
		return;
	if (!midr_nds_local_transport_get(ctx->g1, &local_transport)
	...
	event_add_timer_msec(midr_g1_master(), midr_pm_timeout, ctx,
...
static void midr_pm_probe_timer_fn(struct event *t)
{
	...
	if (ctx->state == MIDR_PROBE_FAST)
		event_add_timer_msec(midr_g1_master(), midr_pm_probe_timer_fn, ctx,
				     MIDR_PM_PROBE_INTERVAL_FAST_MS,
				     &ctx->t_probe);
	else
		event_add_timer(midr_g1_master(), midr_pm_probe_timer_fn, ctx,
 ...
 */
static bool pm_is_known_transport(struct midr_nds *mi,
				  const struct ipaddr *locator)
{
	...
	frr_each (midr_node_hash, &mi->global_view->nodes, entry) {
		...
		if (entry->has_transport_addr
			...
			if (!midr_nds_locator_unique(mi->g1, &entry->node_id,
						      locator))
				return false;
...
static void midr_pm_recv(struct event *t)
{
	...
	event_add_read(midr_g1_master(), midr_pm_recv, g1, mi->pm_sock,
		       &mi->t_pm_read);
	...
	if (!midr_nds_local_transport_get(g1, &local_transport)
...
static void midr_pm_open_sock(struct midr_g1 *g1)
{
	struct midr_nds *mi = g1->midr_nds_info;
	...
	mi->pm_sock = sock;
	event_add_read(midr_g1_master(), midr_pm_recv, g1, sock, &mi->t_pm_read);

	MIDR_LOG("MIDR PM: probe socket ready on %pIA:%u", &local_transport,
		 MIDR_PM_PROBE_PORT);
}

static void midr_pm_close_sock(struct midr_g1 *g1)
{
	struct midr_nds *mi = g1->midr_nds_info;
...
static void midr_pm_probe_timer(struct event *t)
{
	...
	if (!midr_nds_local_transport_get(g1, &local_transport))
		midr_pm_on_transport_addr_unset(g1);
	...
	frr_each (midr_node_hash, &mi->global_view->nodes, entry) {
		...
		if (!entry->is_adjacent)
			continue;
		if (!midr_node_established_peer(g1, &entry->node_id))
			continue;
...
reschedule:
	event_add_timer(midr_g1_master(), midr_pm_probe_timer, g1,
...
int midr_pm_add_target(struct midr_g1 *g1, const struct prefix *node_id,
		       enum midr_node_source source, uint32_t capabilities)
{
	...
	if (!midr_nds_node_transport_get(g1, node_id, &target_transport)) {
	...
	if (ctx && midr_ipaddr_same(&ctx->target_addr, &target_transport))
		return 0;
	if (ctx)
		midr_pm_remove_target(g1, node_id,
	...
	event_add_timer(midr_g1_master(), midr_pm_probe_timer_fn, ctx, 0,
			&ctx->t_probe);
...
int midr_pm_remove_target(struct midr_g1 *g1, const struct prefix *node_id,
			  enum midr_stop_reason reason)
{
	...
	if (ctx->probe_outstanding) {
		...
		midr_nds_on_link_update(g1, node_id, MIDR_LINK_DOWN,
 ...
 */
int midr_pm_remove_all_targets(struct midr_g1 *g1, enum midr_stop_reason reason)
{
	struct midr_nds *mi;
	int n;
 ...
 */
void midr_pm_on_transport_addr_set(struct midr_g1 *g1)
{
	struct midr_nds *mi;
	struct midr_node_entry *entry;
	struct ipaddr local_transport;
	...
	if (mi->pm_sock >= 0
		...
		MIDR_LOG("MIDR PM: rebinding probe socket to %pIA",
			 &local_transport);
		midr_pm_close_sock(g1);
		midr_pm_remove_all_targets(g1, MIDR_STOP_CLUSTER_CHANGE);
	}

	if (mi->pm_sock < 0)
		midr_pm_open_sock(g1);
...
void midr_pm_on_transport_addr_unset(struct midr_g1 *g1)
{
	struct midr_nds *mi;
...
void midr_pm_init(struct midr_g1 *g1)
{
	struct midr_nds *mi;
...
void midr_pm_finish(struct midr_g1 *g1)
{
	struct midr_nds *mi;
