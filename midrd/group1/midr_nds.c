...
DEFINE_MTYPE_STATIC(MIDR_G1, BGP_MIDR, "MIDR instance");
DEFINE_MTYPE_STATIC(MIDR_G1, MIDR_GLOBAL_VIEW, "MIDR global view");
DEFINE_MTYPE_STATIC(MIDR_G1, MIDR_NODE_ENTRY, "MIDR node entry");
DEFINE_MTYPE_STATIC(MIDR_G1, MIDR_LINK_ENTRY, "MIDR link entry");
DEFINE_MTYPE_STATIC(MIDR_G1, MIDR_REP_ENTRY, "MIDR rep directory entry");
DEFINE_MTYPE_STATIC(MIDR_G1, MIDR_BOOTSTRAP_ENTRY, "MIDR bootstrap candidate");
DEFINE_MTYPE_STATIC(MIDR_G1, MIDR_SESSION_EXCLUDE, "MIDR excluded session");
DEFINE_MTYPE_STATIC(MIDR_G1, MIDR_SESSION_LEDGER, "MIDR session ledger entry");
DEFINE_MTYPE_STATIC(MIDR_G1, MIDR_SHUTDOWN_TARGET, "MIDR shutdown target");
DEFINE_MTYPE_STATIC(MIDR_G1, MIDR_MANUAL_SESSION, "MIDR manual session config");
DEFINE_MTYPE_STATIC(MIDR_G1, MIDR_ATTACH_DOWN, "MIDR attach down-pending");
DEFINE_MTYPE_STATIC(MIDR_G1, MIDR_SESSION_DOWN, "MIDR session down-pending");

/* 前向声明：候选池插入（定义在本文件"§8.32 bootstrap 候选"一节，靠近
 * failover 机械件）。种子落库时顺手补插候选池（子稿 §4-1）需要在此之前调它。 */
static void midr_bootstrap_list_add(struct midr_nds *mi,
				    struct ipaddr addr,
...
midr_bootstrap_find(struct midr_nds *mi, struct ipaddr addr,
		    struct listnode **node_out);
/* 同上：确保挂靠达标（定义在挂靠一节，紧挨 attach_count——它得先有定义）。
 * 能力位 setter 与 config_end 回调都要在此之前调它。 */
static void midr_nds_attach_ensure(struct midr_g1 *g1, const char *why);
...
static bool midr_prefix_is_self(struct midr_g1 *g1, const struct prefix *p)
{
	return p->family == AF_INET &&
	       IPV4_ADDR_SAME(&p->u.prefix4, &g1->router_id);
}
 ...
 */
static void midr_maybe_save_bootstrap_seed(struct midr_g1 *g1,
					   const struct midr_node_entry *entry)
{
	 ...
	 * 身份时本兼容路径才会触发。 */
	midr_bootstrap_list_add(g1->midr_nds_info, entry->transport_addr,
...
bool midr_nds_is_session_excluded(struct midr_g1 *g1, struct in_addr rid)
{
	struct midr_nds *mi;
	struct listnode *node;
	struct in_addr *a;

	if (!g1 || !g1->midr_nds_info || !g1->midr_nds_info->session_blacklist)
		return false;
	mi = g1->midr_nds_info;
...
bool midr_nds_session_blacklist_nonempty(struct midr_g1 *g1)
{
	if (!g1 || !g1->midr_nds_info || !g1->midr_nds_info->session_blacklist)
		return false;
	return !list_isempty(g1->midr_nds_info->session_blacklist);
}

void midr_nds_session_exclude_add(struct midr_g1 *g1, struct in_addr rid)
{
	struct midr_nds *mi;
	struct in_addr *a;
	...
	if (midr_nds_is_session_excluded(g1, rid))
		return; /* 已在名单，幂等 */
...
void midr_nds_session_exclude_del(struct midr_g1 *g1, struct in_addr rid)
{
	struct midr_nds *mi;
	struct listnode *node;
	struct in_addr *a;
...
midr_ledger_find(struct midr_nds *mi, struct ipaddr transport)
{
	struct listnode *node;
...
midr_nds_ledger_lookup(struct midr_g1 *g1, struct ipaddr transport)
{
	if (!g1 || !g1->midr_nds_info || !g1->midr_nds_info->session_ledger)
		return NULL;
	return midr_ledger_find(g1->midr_nds_info, transport);
}

void midr_nds_ledger_note(struct midr_g1 *g1, struct ipaddr transport,
...
void midr_nds_ledger_drop(struct midr_g1 *g1, struct ipaddr transport)
{
	struct midr_nds *mi;
	struct listnode *node, *nnode;
	struct midr_session_ledger_entry *e;
	...
	if (!g1 || !g1->midr_nds_info || !g1->midr_nds_info->session_ledger)
		return;
	mi = g1->midr_nds_info;
	midr_admission_forget(g1, transport);
 ...
 */
static void midr_ledger_note_down(struct midr_g1 *g1, struct ipaddr transport)
{
	struct midr_session_ledger_entry *e;

	if (!g1->midr_nds_info || !g1->midr_nds_info->session_ledger)
		return;
	e = midr_ledger_find(g1->midr_nds_info, transport);
	if (!e)
...
static void midr_ledger_note_up(struct midr_g1 *g1, struct ipaddr transport)
{
	struct midr_session_ledger_entry *e;
 ...
 * 个 5 分钟窗口里全断且都没恢复）。
 */
static void midr_nds_ledger_age_scan(struct midr_g1 *g1)
{
	...
	for (i = 0; i < n; i++) {
		...
		midr_ctrl_detach_transport(g1, aged[i].transport,
					   aged[i].rid, false,
 ...
 */
bool midr_discovery_should_peer(struct midr_g1 *g1,
				const struct midr_node_entry *entry)
{
	struct midr_nds *mi = g1->midr_nds_info;
	struct ipaddr local;
	...
	if (!mi || !entry->has_transport_addr ||
	    ...
	    !midr_nds_locator_unique(g1, &entry->node_id,
	...
	if (entry->node_id.family == AF_INET &&
	    midr_nds_is_session_excluded(g1, entry->node_id.u.prefix4)) {
		/*
 ...
 */
static bool midr_nds_edge_is_anchor(struct midr_g1 *g1, uint32_t remote_rid)
{
	struct midr_nds *mi;
...
static void midr_nds_detach_node(struct midr_g1 *g1, struct midr_node_entry *entry,
				 enum midr_stop_reason reason,
				 bool teardown_session)
{
	struct midr_global_view *gv = g1->midr_nds_info->global_view;
	...
	midr_pm_remove_target(g1, &entry->node_id, reason);
	/*
	...
	if (teardown_session)
		midr_ctrl_on_node_remove(g1, entry);
}
 ...
 */
static void midr_nds_on_node_discovered(struct midr_g1 *g1,
					struct midr_node_entry *entry)
{
	if (entry->is_adjacent || !midr_discovery_should_peer(g1, entry))
		return;

	MIDR_FLOW_LOG("MIDR 发现：节点 %pFX 群 %u -> 同群未邻接，建边",
		      &entry->node_id, entry->group_id);
	midr_ctrl_connect(g1, entry, MIDR_SESSION_SAME_GROUP, true);
}
 ...
 */
void midr_nds_node_react(struct midr_g1 *g1, struct midr_node_entry *entry,
			 ...
			 uint32_t prev_gid)
{
	if (!entry->is_self) {
		...
		if (is_new || changed)
			midr_maybe_save_bootstrap_seed(g1, entry);
		...
		if ((is_new || changed) &&
		    midr_nds_link_is_backbone(g1, &entry->node_id))
			midr_nds_report_link_withdraw(g1, &entry->node_id);

		if (is_new) {
			/* 同群就补边（见函数头）；随后统一告知 CL 视图变了。 */
			midr_nds_on_node_discovered(g1, entry);
			midr_nds_notify_cl(g1, MIDR_TRIGGER_NODE_CHANGE);
		} else if (changed) {
			/*
			...
			if (group_changed) {
				...
				bool now_mine =
					midr_discovery_should_peer(g1, entry);
...
static bool
midr_nds_control_locator_usable(struct midr_g1 *g1, const struct prefix *owner,
 ...
 */
void midr_nds_learn_member(struct midr_g1 *g1, struct in_addr rid, as_t asn,
			   struct ipaddr transport, uint32_t group_id)
{
	...
	if (!g1 || !g1->midr_nds_info || rid.s_addr == INADDR_ANY)
		return;

	gv = g1->midr_nds_info->global_view;
	midr_prefix_from_in_addr(&key.node_id, rid);

	entry = midr_node_hash_find(&gv->nodes, &key);
	if (!midr_nds_control_locator_usable(g1, &key.node_id, entry,
					     &transport, "成员名单"))
		return;
	if (!entry) {
	...
	entry->is_self = midr_prefix_is_self(g1, &entry->node_id);
	entry->is_adjacent = true; /* 成员表里的成员就是要建邻居的对象 */

	/* I-1: probe by node_id; PM resolves transport_addr from global_view. */
	midr_pm_add_target(g1, &entry->node_id, MIDR_SRC_BOOTSTRAP,
			   entry->capabilities);
	midr_admission_screen(g1, entry);
}
 ...
 * 各自独立、名字各自说明白自己在干什么。
 */
void midr_nds_learn_anchor_candidate(struct midr_g1 *g1, struct in_addr rid,
 ...
 */
void midr_nds_adopt_group_peer(struct midr_g1 *g1, struct in_addr rid, as_t asn,
			       struct ipaddr transport, uint32_t group_id)
{
	struct midr_global_view *gv;
	struct midr_node_entry key = {};
	...
	midr_pm_add_target(g1, &entry->node_id, MIDR_SRC_GOSSIP,
			   entry->capabilities);
}
 ...
 */
void midr_nds_cleanup_by_transport(struct midr_g1 *g1, struct ipaddr transport,
				   enum midr_stop_reason reason)
{
	struct midr_node_entry *entry;
	...
	frr_each (midr_node_hash, &g1->midr_nds_info->global_view->nodes,
		  entry) {
		if (!entry->has_transport_addr ||
		...
		zlog_info("MIDR 边注销：%pFX 的本群邻居身份已撤（会话没了，停探并不再计入 CL 统计）",
			  &entry->node_id);
		midr_nds_detach_node(g1, entry, reason, false);
		midr_nds_notify_cl(g1, MIDR_TRIGGER_NODE_CHANGE);
		return;
	}
}
 ...
 */
void midr_nds_learn_requester(struct midr_g1 *g1, struct in_addr rid, as_t asn,
			      struct ipaddr transport)
{
	struct midr_global_view *gv;
	struct midr_node_entry key = {};
 ...
 */
void midr_nds_local_node_update(struct midr_g1 *g1)
{
	struct midr_global_view *gv;
	struct midr_node_entry key = {};
	struct midr_node_entry *entry;
	...
	gv = g1->midr_nds_info->global_view;
	midr_prefix_from_in_addr(&key.node_id, g1->router_id);
	...
	entry->asn = g1->as;
	entry->group_id = g1->midr_nds_info->local_group_id;
	entry->capabilities = g1->midr_nds_info->local_capabilities;
	if (g1->midr_nds_info->transport_active) {
		entry->transport_addr =
			g1->midr_nds_info->active_transport_addr;
		entry->has_transport_addr = true;
	} else {
...
bool midr_nds_is_bootstrap(struct midr_g1 *g1)
{
	return g1 && g1->midr_nds_info &&
	       !!(g1->midr_nds_info->local_capabilities & MIDR_CAP_BOOTSTRAP);
}

/* 判据与用途见头文件。三条判据任一命中即真。 */
bool midr_nds_link_is_backbone(struct midr_g1 *g1,
			       const struct prefix *remote_node_id)
{
	...
	if (!g1 || !g1->midr_nds_info || !remote_node_id ||
 ...
 */
void midr_nds_bootstrap_enforce(struct midr_g1 *g1)
{
	struct midr_nds *mi;
	...
	if (mi->local_capabilities & MIDR_CAP_GROUP_REP) {
		...
		midr_nds_set_capability(g1, mi->local_capabilities &
						     ~MIDR_CAP_GROUP_REP);
	}
	...
	if (mi->config_group_id != 0 || mi->local_group_id != 0) {
		...
		midr_nds_set_group_id(g1, 0);
	}
}

/* bgp_config_end 钩子的适配壳（hook 要求 int 返回值）。 */
static int midr_nds_bootstrap_after_cfg(struct midr_g1 *g1)
{
	midr_nds_bootstrap_enforce(g1);
	return 0;
}

void midr_nds_set_capability(struct midr_g1 *g1, uint32_t new_caps)
{
	struct midr_nds *mi;
	bool was_rep, is_rep;
	...
	if (is_boot && !was_boot)
		midr_nds_facts_withdraw_all_links(g1);
	midr_nds_report_node(g1, MIDR_ORIGIN_CAP_UPDATE);
	...
	if (is_rep) {
		...
		if (midr_g1_config_inprocess()) {
			...
			midr_nds_attach_ensure(g1, was_rep ? "仍是群代表"
							    : "成为群代表");
		}
	} else if (was_rep) {
		zlog_info("MIDR 挂靠：本机卸任群代表，拆除挂靠会话");
		midr_nds_attach_detach_all(g1);
	}
 ...
 */
void midr_nds_attach_detach_all(struct midr_g1 *g1)
{
	struct midr_nds *mi;
	struct listnode *node;
	struct midr_session_ledger_entry *e;
	...
	for (i = 0; i < n; i++)
		midr_ctrl_detach_transport(g1, doomed[i], rids[i], false,
					   MIDR_STOP_CLUSTER_CHANGE);
 ...
 * 跳过、压根没起探，这里直接返回即可。
 */
static void midr_stop_rep_probe_entry(struct midr_g1 *g1,
				      const struct midr_rep_entry *r)
{
	...
	midr_nds_detach_node(g1, rep_entry, MIDR_STOP_CLUSTER_CHANGE, false);
}

static uint32_t midr_group_reconverge(struct midr_g1 *g1, uint32_t new_gid)
{
	struct midr_nds *mi = g1->midr_nds_info;
	...
	/* 1. 改群号 + 重通告（先于建连）。 */
	midr_originate_group_update(g1, new_gid, old_gid);

	/* 2. 与新群成员建连（离群 new_gid==0 时无成员可连，跳过）。 */
	if (new_gid != 0)
		connected = midr_ctrl_connect_group(g1, new_gid,
	...
	frr_each (midr_node_hash, &mi->global_view->nodes, entry) {
		if (entry->is_self || !entry->is_adjacent)
			continue;
		if (midr_discovery_should_peer(g1, entry))
			continue; /* 仍属本群，留着 */
		/*
		...
		midr_nds_detach_node(g1, entry, MIDR_STOP_CLUSTER_CHANGE, true);
		detached++;
	}
	...
	for (ALL_LIST_ELEMENTS_RO(mi->rep_dir, rn, r)) {
		...
		midr_stop_rep_probe_entry(g1, r);
		reps_stopped++;
	}
 ...
 */
void midr_nds_anchor_ctx_clear(struct midr_g1 *g1)
{
	struct midr_nds *mi;
	...
	midr_admission_forget_reason(g1, MIDR_SESSION_CL_ANCHOR);
...
void midr_nds_set_group_id(struct midr_g1 *g1, uint32_t new_gid)
{
	struct midr_nds *mi;
	...
	midr_group_reconverge(g1, new_gid);
}
...
static unsigned int midr_established_session_count(struct midr_g1 *g1)
{
 ...
 */
static void midr_isolation_check(struct midr_g1 *g1)
{
	struct midr_nds *mi = g1->midr_nds_info;
	...
	if (midr_established_session_count(g1) > 0) {
		mi->isolated_ticks = 0;
		return;
	}
	...
	mi->isolated_ticks = 0;
	midr_nds_notify_cl(g1, MIDR_TRIGGER_ISOLATED);
}
 ...
 */
static void midr_nds_reconnect_missing_peers(struct midr_g1 *g1)
{
	struct midr_nds *mi = g1->midr_nds_info;
	struct midr_node_entry *entry;
	...
	frr_each (midr_node_hash, &mi->global_view->nodes, entry) {
		...
		if (!midr_ipaddr_to_sockunion(&entry->transport_addr, &su))
			continue;
		peer = midr_g1_peer_lookup_su(g1, &su);
	...
	for (i = 0; i < n; i++) {
		zlog_info("MIDR 补边兜底：同群 %pFX 的 overlay 会话未通，重连",
			  &missing[i]->node_id);
		midr_ctrl_connect(g1, missing[i], MIDR_SESSION_SAME_GROUP,
				  true);
	}
}

/* 注册函数定义在文件后半（回调实现旁边），periodic_sync 的兜底重试要用。 */
static void midr_nds_remote_view_register(struct midr_g1 *g1);
...
static void midr_periodic_sync_timer(struct event *t)
{
	 ...
	 */
	midr_nds_remote_view_register(g1);
	...
	if (mi->transport_addr_set && !mi->transport_active &&
		...
		(void)midr_nds_transport_reconcile(g1);

	midr_nds_notify_cl(g1, MIDR_TRIGGER_PERIODIC_SYNC);
	...
	midr_nds_ledger_age_scan(g1);

	/* 失联自救：会话全断时没有任何别的机制会反应（见函数头注释）。 */
	midr_isolation_check(g1);

	/* 同群补边兜底：收包侧那条路只在状态跳变时触发，失败就没有下一次。 */
	midr_nds_reconnect_missing_peers(g1);

	event_add_timer(midr_g1_master(), midr_periodic_sync_timer, g1,
 ...
 */
struct midr_g1_peer *midr_node_established_peer(struct midr_g1 *g1,
				       const struct prefix *node_id)
{
	struct midr_g1_peer *peer;
	struct listnode *node;

	if (!g1 || !node_id || node_id->family != AF_INET ||
...
void midr_nds_on_link_update(struct midr_g1 *g1, const struct prefix *node_id,
			     ...
			     const struct midr_nds_link_metrics *long_term)
{
	...
	if (!g1 || !g1->midr_nds_info || !node_id)
		return;
...
void midr_nds_notify_cl(struct midr_g1 *g1, enum midr_trigger_type trigger)
{
	struct midr_nds *mi = g1->midr_nds_info;
	...
	if (mi && mi->cl_callback)
		mi->cl_callback(g1, trigger, mi->global_view);
}
...
void midr_originate_group_update(struct midr_g1 *g1, uint32_t new_group_id,
				 uint32_t old_group_id)
{
	...
	if (new_group_id != old_group_id)
		midr_admission_forget_reason(g1, MIDR_SESSION_SAME_GROUP);
	...
	g1->midr_nds_info->local_group_id = new_group_id;
	/* B1：唯一写手，见 struct bgp_midr_nds.group_settled_at 处注释。 */
	g1->midr_nds_info->group_settled_at = monotime(NULL);
 ...
 * "旧群号 + 我是代表"的错误组合。
 */
static void midr_join_settle_group(struct midr_g1 *g1, uint32_t gid)
{
	struct midr_nds *mi = g1->midr_nds_info;
	struct list *members;
	bool alone;
	...
	members = list_new();
	midr_group_members(g1, gid, members);
	alone = list_isempty(members);
	list_delete(&members);

	midr_group_reconverge(g1, gid);
 ...
 */
static void midr_join_round_start(struct midr_g1 *g1); /* forward */

static bool midr_rep_identity_resolve(
	struct midr_g1 *g1, const struct midr_rep_identity *identity,
	struct ipaddr *transport)
{
	struct midr_nds *mi;
	...
	if (!g1 || !g1->midr_nds_info || !identity || !transport ||
...
void midr_nds_on_cluster_decision(struct midr_g1 *g1,
				  const struct midr_cluster_decision *decision)
{
	struct midr_nds *mi;

	if (!g1 || !g1->midr_nds_info || !decision)
		return;
	...
	case MIDR_DECISION_RECOMMEND: {
		...
		if (!midr_rep_identity_resolve(g1, &recommended,
		...
		if (mi->config_group_id != 0) {
			...
			if (!r) {
				...
				zlog_info("MIDR I-7：不采纳 RECOMMEND 群 %u（本地配置群 %u）；目录中无群 %u 的代表，跳过探成员直接按配置群号落定",
					  ...
					  mi->config_group_id,
					  mi->config_group_id);
				midr_join_settle_group(g1, mi->config_group_id);
				break;
			}
			...
			if (!midr_rep_identity_resolve(g1, &configured,
						       &rep_transport)) {
		...
		midr_ctrl_send_member_request(g1, rep_transport, target_gid);
		...
		mi->anchor_group_id[1] = 0;
		{
			...
			for (i = 0; i < npool && slot < 2; i++) {
				...
				midr_ctrl_send_member_request(g1,
							      pool_transport[i],
							      pool_gid[i]);
	...
	case MIDR_DECISION_JOIN:
		 ...
		 */
		mi->join_members =
			midr_group_reconverge(g1, decision->new_group_id);
	...
	case MIDR_DECISION_SPLIT:
		midr_originate_group_update(g1, decision->new_group_id,
	...
	case MIDR_DECISION_CREATE: {
		 ...
		 * 经引导节点目录发现本群）。
		 */
		midr_join_settle_group(g1, create_gid);
	...
	case MIDR_DECISION_REP_RESIGN:
		...
		if (mi->transport_active &&
			...
			midr_rep_dir_del(g1, mi->local_group_id,
	...
	case MIDR_DECISION_ANCHOR: {
		...
		if (decision->evidence)
			for (ALL_LIST_ELEMENTS_RO(decision->evidence, en, ev)) {
				...
				enum midr_admission_result admission = midr_ctrl_connect(g1, entry,
...
void midr_rep_dir_add(struct midr_g1 *g1, uint32_t group_id,
		      ...
		      struct in_addr rep_rid)
{
	...
	if (!g1 || !g1->midr_nds_info || !g1->midr_nds_info->rep_dir ||
...
bool midr_rep_dir_del(struct midr_g1 *g1, uint32_t group_id,
		      struct ipaddr rep_transport)
{
	struct midr_nds *mi;
	struct listnode *node, *nnode;
...
void midr_rep_dir_clear(struct midr_g1 *g1)
{
	struct midr_nds *mi;
	struct listnode *node, *nnode;
	struct midr_rep_entry *r;
...
struct midr_rep_entry *midr_rep_dir_find_group(struct midr_g1 *g1,
					       uint32_t group_id)
{
	struct midr_nds *mi;
	struct listnode *node;
 ...
 */
void midr_group_members(struct midr_g1 *g1, uint32_t group_id, struct list *out)
{
	struct midr_node_entry *entry;

	if (!g1 || !g1->midr_nds_info || !out)
		return;

	frr_each (midr_node_hash, &g1->midr_nds_info->global_view->nodes, entry) {
		if (entry->is_self)
			continue;
 ...
 */
void midr_rep_candidates(struct midr_g1 *g1, struct list *out)
{
	struct midr_node_entry *entry;
...
struct midr_global_view *midr_get_global_view(struct midr_g1 *g1)
{
	if (!g1 || !g1->midr_nds_info)
		return NULL;
	return g1->midr_nds_info->global_view;
}

uint32_t midr_local_group_id(struct midr_g1 *g1)
{
	if (!g1 || !g1->midr_nds_info)
		return 0;
	return g1->midr_nds_info->local_group_id;
}

bool midr_node_group_id(struct midr_g1 *g1, const struct prefix *node_id,
			uint32_t *out)
{
	struct midr_node_entry key = {};
	struct midr_node_entry *entry;

	if (!g1 || !g1->midr_nds_info || !node_id || !out)
		return false;

	prefix_copy(&key.node_id, node_id);
	entry = midr_node_hash_find(&g1->midr_nds_info->global_view->nodes, &key);
	if (!entry)
		return false;
...
bool midr_nds_local_transport_get(const struct midr_g1 *g1,
				  struct ipaddr *transport)
{
	const struct midr_nds *mi;
	...
	if (!mi->transport_active ||
	    !midr_ipaddr_valid_locator(&mi->active_transport_addr))
		return false;
	midr_prefix_from_in_addr(&self_id, g1->router_id);
	if (!midr_nds_locator_unique(g1, &self_id, &mi->active_transport_addr))
		return false;
...
bool midr_nds_node_transport_get(const struct midr_g1 *g1,
				 const struct prefix *node_id,
				 struct ipaddr *transport)
{
	struct midr_node_entry key = {};
	...
	if (!transport)
		return false;
	SET_IPADDR_NONE(transport);
	if (!g1 || !node_id || !g1->midr_nds_info ||
	    !g1->midr_nds_info->global_view)
		return false;

	prefix_copy(&key.node_id, node_id);
	entry = midr_node_hash_find(&g1->midr_nds_info->global_view->nodes,
				    &key);
	if (!entry || !entry->has_transport_addr ||
	    !midr_ipaddr_valid_locator(&entry->transport_addr) ||
	    !midr_nds_local_transport_get(g1, &local) ||
	    ipaddr_family(&local) != ipaddr_family(&entry->transport_addr) ||
	    !midr_nds_locator_unique(g1, node_id, &entry->transport_addr))
		return false;
...
midr_manual_session_find(struct midr_nds *mi, struct ipaddr transport)
{
	struct listnode *node;
...
static bool midr_manual_session_note_identity(struct midr_g1 *g1,
					      struct ipaddr transport,
					      struct in_addr remote_rid)
{
	...
	if (!session)
		return true;
	if (remote_rid.s_addr == INADDR_ANY ||
	    midr_nds_is_session_excluded(g1, remote_rid))
		return false;

	midr_prefix_from_in_addr(&key.node_id, remote_rid);
	existing = midr_node_hash_find(&mi->global_view->nodes, &key);
	if (!midr_nds_control_locator_usable(g1, &key.node_id, existing,
					     &transport, "手工会话身份回填"))
		return false;
...
void midr_nds_manual_session_set(struct midr_g1 *g1, struct ipaddr transport,
				 as_t remote_asn,
				 struct in_addr remote_rid)
{
	struct midr_nds *mi;
...
bool midr_nds_manual_session_del(struct midr_g1 *g1, struct ipaddr transport)
{
	struct midr_nds *mi;
	struct listnode *node, *nnode;
	struct midr_manual_session *session;
...
void midr_nds_manual_sessions_restore(struct midr_g1 *g1)
{
	struct midr_nds *mi;
	struct listnode *node;
	struct midr_manual_session *session;
	...
	for (ALL_LIST_ELEMENTS_RO(mi->manual_sessions, node, session)) {
		...
		if (midr_admission_is_manual(g1, session->transport))
			continue;
		if (midr_ipaddr_to_sockunion(&session->transport, &su)) {
			peer = midr_g1_peer_lookup_su(g1, &su);
			ledger = midr_nds_ledger_lookup(g1, session->transport);
			if (peer && peer->established &&
		...
		resolved_rid = midr_nds_rid_by_transport(g1,
						    session->transport);
		...
		midr_ctrl_connect(g1, &target, MIDR_SESSION_MANUAL, false);
	}
}

bool midr_nds_locator_unique(const struct midr_g1 *g1,
			     ...
			     const struct ipaddr *transport)
{
	...
	if (!g1 || !g1->midr_nds_info ||
	    !g1->midr_nds_info->global_view || !owner ||
 ...
 */
struct in_addr midr_nds_rid_by_transport(struct midr_g1 *g1,
					 struct ipaddr transport)
{
 ...
 * 查不到返回 0。
 */
struct in_addr midr_nds_bootstrap_rid_by_transport(struct midr_g1 *g1,
						   struct ipaddr transport)
{
	struct in_addr zero = { .s_addr = INADDR_ANY };
	...
	b = midr_bootstrap_find(g1->midr_nds_info, transport, NULL);
	return b ? b->rid : zero;
}
 ...
 * 散出去。
 */
bool midr_nds_detach_by_locator(struct midr_g1 *g1, struct ipaddr addr)
{
	struct midr_node_entry *entry;
...
static unsigned int midr_bootstrap_index(struct midr_nds *mi,
 ...
 * 追加到尾，维持"手配在前、种子在后"。
 */
static void midr_bootstrap_list_add(struct midr_nds *mi, struct ipaddr addr,
				    as_t asn, struct in_addr rid,
				    enum midr_bootstrap_source source)
{
	struct midr_bootstrap_entry *b;
...
static void midr_bootstrap_start_attempt(struct midr_g1 *g1)
{
	struct midr_nds *mi = g1->midr_nds_info;
	struct midr_bootstrap_entry *b;
	struct ipaddr local;
	...
	if (!midr_nds_local_transport_get(g1, &local))
		return;
	...
	b = listgetdata(mi->bootstrap_cur);
	midr_ctrl_send_rep_request(g1, b->transport);
 ...
 */
static void midr_join_round_start(struct midr_g1 *g1)
{
	struct midr_nds *mi = g1->midr_nds_info;
	struct listnode *node;
	struct midr_bootstrap_entry *b;
	...
	midr_bootstrap_start_attempt(g1);
}
					...
					void *arg)
{
	...
	if (!rid || inet_pton(AF_INET, rid, &rid_addr) != 1)
		return;
	midr_bootstrap_list_add(g1->midr_nds_info, locator, (as_t)asn,
...
void midr_join_via_bootstrap(struct midr_g1 *g1, const union sockunion *su,
			     as_t asn, struct in_addr rid)
{
	struct midr_nds *mi;
	struct ipaddr locator;

	if (!g1 || !g1->midr_nds_info || !su)
		return;
 ...
 */
void midr_join_bootstrap_failed(struct midr_g1 *g1, struct ipaddr failed)
{
	struct midr_nds *mi;
	struct midr_bootstrap_entry *b;
...
bool midr_bootstrap_list_del(struct midr_g1 *g1, struct ipaddr addr)
{
	struct midr_nds *mi;
...
void midr_bootstrap_clear(struct midr_g1 *g1)
{
	struct midr_nds *mi;
	struct listnode *node, *nnode;
	struct midr_bootstrap_entry *b;
 ...
 */
static unsigned int midr_shutdown_teardown_sessions(struct midr_g1 *g1,
						    unsigned int *manual_out)
{
	...
	for (ALL_LIST_ELEMENTS_RO(mi->shutdown_targets, node, target)) {
		...
		midr_ctrl_detach_transport(g1, target->transport,
...
static unsigned int midr_shutdown_session_count(struct midr_g1 *g1,
						 unsigned int *manual_out)
{
	struct midr_nds *mi;
	struct midr_shutdown_target *target;
	...
	if (manual_out)
		*manual_out = 0;
	if (!g1 || !(mi = g1->midr_nds_info) || !mi->shutdown_targets)
		return 0;
	for (ALL_LIST_ELEMENTS_RO(mi->shutdown_targets, node, target)) {
		count++;
		if (target->reason == MIDR_SESSION_MANUAL)
...
static void midr_shutdown_targets_clear(struct midr_g1 *g1)
{
	struct midr_nds *mi;
...
static bool midr_shutdown_peer_established(struct midr_g1 *g1,
					   const struct ipaddr *transport)
{
	struct midr_g1_peer *peer;

	if (!g1 || !transport || !midr_ipaddr_valid_locator(transport))
		return false;
	peer = midr_g1_peer_lookup(g1, transport);
	return peer && peer->established;
}

static void midr_shutdown_targets_snapshot(struct midr_g1 *g1)
{
	struct midr_nds *mi;
	...
	if (!g1 || !(mi = g1->midr_nds_info) || !mi->session_ledger ||
	    !mi->shutdown_targets)
		return;
	midr_shutdown_targets_clear(g1);
	for (ALL_LIST_ELEMENTS_RO(mi->session_ledger, node, entry)) {
		if (!midr_shutdown_peer_established(g1, &entry->transport))
			continue;
...
static void midr_shutdown_teardown_cb(struct event *t)
{
	...
	now = time(NULL);
	/* midrd floods the withdrawals; its public API does not report when
	 * they are written, so hold the sessions for the fixed delay. */
	if (now < mi->shutdown_teardown_started + MIDR_SHUTDOWN_TEARDOWN_DELAY) {
		event_add_timer_msec(midr_g1_master(), midr_shutdown_teardown_cb, g1,
				     MIDR_SHUTDOWN_TEARDOWN_POLL_MSEC,
				     &mi->t_shutdown_teardown);
		return;
	}

	n = midr_shutdown_teardown_sessions(g1, &manual);
	zlog_info("MIDR 退网：撤销写出等待结束，拆除会话 %u 条（手配 %u 条）",
		  n, manual);
}

unsigned int midr_nds_shutdown_enter(struct midr_g1 *g1)
{
	struct midr_nds *mi;
	struct midr_node_entry *entry;
 ...
 * 敲的命令、与手配同级，留一条还在收发 MIDR-LS 的会话，对上层（CL / 第二组
	 ...
	 */
	n = midr_shutdown_session_count(g1, &manual);
	 ...
	 */
	stopped = midr_pm_remove_all_targets(g1, MIDR_STOP_GRACEFUL_SHUTDOWN);
	...
	frr_each_safe (midr_node_hash, &mi->global_view->nodes, entry) {
		if (entry->is_self)
			continue;
		midr_nds_detach_node(g1, entry, MIDR_STOP_GRACEFUL_SHUTDOWN,
				     false);
	...
	mi->join_group_id = 0;
	mi->bootstrap_cur = NULL;
	midr_nds_anchor_ctx_clear(g1); /* 挂点①（配套一） */
...
bool midr_nds_shutdown_exit(struct midr_g1 *g1)
{
	struct midr_nds *mi;
	...
	if (mi->t_shutdown_teardown) {
		...
		torn = midr_shutdown_teardown_sessions(g1, &manual);
	...
	/* ② 重新通告自己（群号 = 退网时回落的配置值）。 */
	midr_nds_report_node(g1, MIDR_ORIGIN_REJOIN);
	/* 手工会话是配置意图，不随退网时的运行态 ledger 一起消失。 */
	midr_nds_manual_sessions_restore(g1);
...
static bool midr_nds_transport_session_up(struct midr_g1 *g1,
					  struct ipaddr transport)
{
	struct prefix p;
	union sockunion su;
...
void midr_nds_bootstrap_learn(struct midr_g1 *g1, struct ipaddr transport,
			      as_t asn, struct in_addr rid)
{
	struct midr_nds *mi;
	struct midr_bootstrap_entry *b;
 ...
 */
void midr_nds_bootstrap_list_begin(struct midr_g1 *g1)
{
	struct midr_nds *mi;
	struct listnode *node;
	struct midr_bootstrap_entry *b;
...
void midr_nds_on_bootstrap_list(struct midr_g1 *g1, struct ipaddr src,
				unsigned int count)
{
	struct midr_nds *mi;
 ...
 */
static unsigned int midr_nds_attach_count(struct midr_g1 *g1)
{
	struct midr_nds *mi = g1->midr_nds_info;
	struct listnode *node;
	struct midr_session_ledger_entry *e;
 ...
 */
static unsigned int midr_attach_pick_pass(struct midr_g1 *g1,
					  ...
					  bool second)
{
	...
	if (!n || !want || !midr_nds_local_transport_get(g1, &local))
		return 0;
	 ...
	 */
	start = jhash_1word(ntohl(g1->router_id.s_addr), 0x4d494452) % n;
	...
	for (i = 0; i < n && picked < want; i++) {
		 ...
		 */
		if (cand->attach_failed || midr_admission_unavailable(g1, cand->transport))
			continue;

		led = midr_nds_ledger_lookup(g1, cand->transport);
		/*
		...
		if ((led && led->reason == MIDR_SESSION_MANUAL) ||
		    midr_admission_is_manual(g1, cand->transport)) {
		...
		enum midr_admission_result admission =
			midr_ctrl_connect(g1, &e, MIDR_SESSION_ATTACH, true);
		 ...
		 */
		if (second)
			midr_ctrl_set_retx_budget(g1, cand->transport,
...
void midr_nds_attach_pick(struct midr_g1 *g1)
{
	midr_nds_attach_pick_from(g1, MIDR_ATTACH_BATCH_FIRST);
}

void midr_nds_attach_pick_from(struct midr_g1 *g1, enum midr_attach_batch from)
{
	struct midr_nds *mi;
	struct listnode *node;
	struct midr_bootstrap_entry *b;
	...
	if (from == MIDR_ATTACH_BATCH_FIRST)
		picked = midr_attach_pick_pass(g1, first, n1, want, false);
	if (picked < want)
		picked += midr_attach_pick_pass(g1, second, n2, want - picked,
						true);
 ...
 */
enum midr_attach_batch midr_nds_attach_mark_failed(struct midr_g1 *g1,
						   struct ipaddr transport)
{
	struct midr_bootstrap_entry *b;
	...
	if (!g1->midr_nds_info->bootstrap_list_seen || b->in_last_list)
		return MIDR_ATTACH_BATCH_FIRST;
 ...
 * K=2 留备份的意义。
 */
void midr_nds_attach_on_session_down(struct midr_g1 *g1, struct ipaddr transport)
{
	struct midr_nds *mi;
	...
	zlog_info("MIDR 挂靠：到引导 %pIA（rid %pI4）的挂靠会话掉线——先拆旧边，再拉新名单重挑",
		  &transport, &rid);
	midr_ctrl_detach_transport(g1, transport, rid, false,
				   MIDR_STOP_SESSION_DOWN);
...
static void midr_nds_bootstrap_probe_attempt(struct midr_g1 *g1)
{
	struct midr_nds *mi = g1->midr_nds_info;
	struct midr_bootstrap_entry *b;
	struct ipaddr local;
	...
	b = listgetdata(mi->bootstrap_probe_cur);
	midr_ctrl_send_bootstrap_list_request(g1, b->transport);
...
void midr_nds_bootstrap_list_fetch(struct midr_g1 *g1)
{
	struct midr_nds *mi;
	struct listnode *node;
	struct midr_session_ledger_entry *e;
	...
	for (ALL_LIST_ELEMENTS_RO(mi->session_ledger, node, e)) {
		if (e->reason != MIDR_SESSION_ATTACH)
			continue;
		if (!midr_nds_transport_session_up(g1, e->transport))
			continue;
		midr_ctrl_send_bootstrap_list_request(g1, e->transport);
	...
	MIDR_LOG("MIDR 引导名单：无挂靠会话可问，退候选清单挨个试（%u 个候选）",
		 listcount(mi->bootstrap_list));
	midr_nds_bootstrap_probe_attempt(g1);
}

void midr_nds_bootstrap_list_failed(struct midr_g1 *g1, struct ipaddr failed)
{
	struct midr_nds *mi;
	struct midr_bootstrap_entry *b;
...
void midr_join_on_rep_list(struct midr_g1 *g1)
{
	struct midr_nds *mi;
	struct listnode *node;
	struct midr_rep_entry *r;
	...
	for (ALL_LIST_ELEMENTS_RO(mi->rep_dir, node, r)) {
		...
		if (IPV4_ADDR_SAME(&r->rep_rid, &g1->router_id))
			continue;
		 ...
		 */
		midr_ctrl_send_announce(g1, r->rep_transport);

		midr_pm_add_target(g1, &locator, MIDR_SRC_BOOTSTRAP, 0);
		midr_admission_screen(g1, entry);
 ...
 *   failover），谁也不靠猜秒数。〕
 */
static int midr_nds_attach_after_cfg(struct midr_g1 *g1)
{
	/* 判据与触发原因都收在 ensure 里（批 6 起三处共用：本回调、能力位 setter、
	 * join 落定），这里只负责在对的时机叫它一次；空指针也由它挡。 */
	midr_nds_attach_ensure(g1, "配置读完（bgp_config_end）");
	return 0;
}
...
static void midr_attach_reap_cb(struct event *t)
{
	...
	for (ALL_LIST_ELEMENTS(mi->attach_down_pending, node, nnode, tr)) {
		...
		midr_nds_attach_on_session_down(g1, transport);
	}
}
 ...
 */
static void midr_nds_attach_note_down(struct midr_g1 *g1, struct ipaddr transport)
{
	struct midr_nds *mi = g1->midr_nds_info;
	const struct midr_session_ledger_entry *e;
	struct ipaddr *slot;
	...
	*slot = transport;
	...
	event_add_event(midr_g1_master(), midr_attach_reap_cb, g1, 0,
			&mi->t_attach_reap);
}
 ...
 * 制造一条上层看来已经可度量的链路。
 */
static void midr_nds_report_link_on_established(struct midr_g1 *g1,
...
static void midr_session_reap_cb(struct event *t)
{
	...
	for (ALL_LIST_ELEMENTS(mi->session_down_pending, node, nnode, sd)) {
		...
		if (cur.invalid_identity) {
			...
			(void)midr_nds_detach_by_locator(g1, cur.transport);
			midr_nds_ledger_drop(g1, cur.transport);
			midr_ctrl_forget_target(g1, cur.transport);
			continue;
		}
...
/*
 * 钩子 (b) 与 B1 断连老化共用的触发源：会话状态变化。bgpd 里挂在 FRR 的
 * peer_status_changed 钩子上；midrd 里由 midr_g1.c 在 midrd 通知会话起落时调用。
 ...
 */
void midr_nds_session_status(struct midr_g1_peer *peer, bool was_established)
{
	struct ipaddr transport;

	if (!peer || !peer->g1 || !peer->g1->midr_nds_info)
		return;
	if (peer->g1->midr_nds_info->transport_reconfiguring)
		return;
	...
	if (peer->established) {
		if (!midr_manual_session_note_identity(peer->g1, transport,
						      peer->remote_id)) {
			...
			listnode_add(mi->session_down_pending, slot);
			event_add_event(midr_g1_master(), midr_session_reap_cb,
					peer->g1, 0, &mi->t_session_reap);
			return;
		}
		midr_ledger_note_up(peer->g1, transport);
		midr_nds_report_link_on_established(peer->g1, transport);
		return;
	}
...
static struct midr_g1 *midr_nds_remote_bgp(void)
{
	struct midr_g1 *g1 = midr_g1_get();

	return (g1 && g1->midr_nds_info) ? g1 : NULL;
}

/* 撤销时记一笔（虚报观察探针）。同 rid 只留最近一次。 */
static void midr_nds_remote_note_withdraw(struct midr_g1 *g1, struct in_addr rid)
{
	struct midr_nds *mi = g1->midr_nds_info;
	struct listnode *node, *nnode;
	...
	w = XCALLOC(MTYPE_MIDR_G1, sizeof(*w));
	w->rid = rid;
	w->at = monotime(NULL);
...
static void midr_nds_remote_seen_update(struct midr_g1 *g1, struct in_addr rid)
{
	struct midr_nds *mi = g1->midr_nds_info;
	struct listnode *node, *nnode;
	struct midr_remote_withdrawn *w;
...
static void midr_nds_remote_clear_deferred(struct midr_nds *mi,
...
static void midr_nds_remote_rekey_rep_dir(struct midr_nds *mi,
					  struct in_addr rid,
...
static void midr_nds_remote_rekey_bootstraps(struct midr_g1 *g1,
					     struct in_addr rid,
					     const struct ipaddr *old_transport,
					     const struct ipaddr *new_transport)
{
...
static bool midr_nds_remote_take_ledger(struct midr_g1 *g1,
					struct ipaddr old_transport,
					struct midr_remote_locator_intent *intent)
{
	...
	ledger = midr_nds_ledger_lookup(g1, old_transport);
	if (!ledger)
		return false;
	intent->present = true;
	...
	midr_nds_ledger_drop(g1, old_transport);
	return true;
}

static void midr_nds_remote_restore_intent(
	struct midr_g1 *g1, struct midr_node_entry *entry,
	const struct midr_remote_locator_intent *intent)
{
	...
	if (intent->reason == MIDR_SESSION_SAME_GROUP &&
	    (!entry->group_id ||
	     ...
	     !midr_nds_locator_unique(g1, &entry->node_id,
				      &entry->transport_addr)))
		return;
	if (midr_nds_is_session_excluded(g1, entry->node_id.u.prefix4))
		return;
	...
	midr_nds_ledger_note(g1, entry->transport_addr, intent->reason,
			     entry->node_id.u.prefix4,
	...
	midr_ctrl_connect(g1, &target, intent->reason, send_nudge);
}

static void midr_nds_remote_node_update(const struct midr_remote_node_info *node)
{
	struct midr_g1 *g1 = midr_nds_remote_bgp();
	struct midr_global_view *gv;
	struct midr_node_entry key = {};
	struct midr_node_entry *entry;
	struct in_addr rid;
	...
	if (has_transport && g1->midr_nds_info->transport_addr_set &&
	    ipaddr_family(&transport) !=
		    ipaddr_family(&g1->midr_nds_info->local_transport_addr)) {
	...
	if (midr_prefix_is_self(g1, &key.node_id))
		return;

	g1->midr_nds_info->remote_node_events++;
	...
	if (has_transport &&
	    ((g1->midr_nds_info->transport_active &&
	      midr_ipaddr_same(&transport,
			       &g1->midr_nds_info->active_transport_addr)) ||
	     !midr_nds_locator_unique(g1, &entry->node_id, &transport))) {
	...
	if (locator_changed) {
		changed = true;
		if (old_has_transport) {
			(void)midr_nds_remote_take_ledger(g1, old_transport,
						  &intent);
			midr_nds_detach_node(g1, entry, MIDR_STOP_SESSION_DOWN,
					     false);
			midr_ctrl_forget_target(g1, old_transport);
			midr_nds_remote_clear_deferred(g1->midr_nds_info,
						       old_transport);
		}
		...
		midr_nds_remote_rekey_rep_dir(g1->midr_nds_info, rid,
					       &old_transport,
					       has_transport && !locator_conflict
						       ? &transport : NULL);
		midr_nds_remote_rekey_bootstraps(
			g1, rid, &old_transport,
	...
	if (locator_conflict) {
		...
		frr_each (midr_node_hash, &gv->nodes, claimant) {
			if (claimant->is_self || !claimant->has_transport_addr ||
				...
				continue;
			midr_nds_detach_node(g1, claimant, MIDR_STOP_SESSION_DOWN,
					     false);
			midr_nds_remote_rekey_rep_dir(g1->midr_nds_info,
				claimant->node_id.u.prefix4, &transport, NULL);
			midr_nds_remote_rekey_bootstraps(
				g1, claimant->node_id.u.prefix4, &transport, NULL);
		}
		midr_nds_ledger_drop(g1, transport);
		midr_ctrl_forget_target(g1, transport);
		midr_nds_remote_clear_deferred(g1->midr_nds_info, transport);
	}
	...
	midr_nds_remote_seen_update(g1, rid);

	midr_nds_node_react(g1, entry, is_new, changed, group_changed,
			    prev_gid);
	if (locator_changed)
		midr_nds_remote_restore_intent(g1, entry, &intent);
	if (locator_conflict && g1->midr_nds_info->transport_active &&
	    midr_ipaddr_same(&transport,
			     &g1->midr_nds_info->active_transport_addr))
		(void)midr_nds_transport_reconcile(g1);
}
...
static void midr_nds_remote_node_withdraw(uint32_t node_id, uint64_t ls_sequence)
{
	...
	midr_nds_remote_note_withdraw(g1, rid);
	 ...
	 * 删条目 / 通知 CL），只是叫醒者换了。 */
	midr_nds_detach_node(g1, entry, MIDR_STOP_GRACEFUL_SHUTDOWN, true);
	midr_node_hash_del(&gv->nodes, entry);
	XFREE(MTYPE_MIDR_NODE_ENTRY, entry);
	midr_nds_notify_cl(g1, MIDR_TRIGGER_NODE_CHANGE);
}
...
static int midr_nds_remote_view_after_cfg(struct midr_g1 *g1)
{
	midr_nds_remote_view_register(g1);
	return 0;
}

static void midr_nds_transport_clear_deferred(struct midr_nds *mi)
{
	struct listnode *node, *nnode;
	struct ipaddr *transport;
	struct midr_session_down *down;
...
static void midr_nds_transport_clear_measurements(struct midr_nds *mi)
{
	struct listnode *node, *nnode;
	struct midr_link_entry *link;
...
int midr_nds_transport_reconcile(struct midr_g1 *g1)
{
	struct midr_nds *mi;
	struct prefix self_id;
	struct midr_g1_peer **doomed = NULL;
	struct midr_g1_peer *peer;
	...
	if (want_transport) {
		...
		if (!midr_ipaddr_valid_locator(&desired))
			return -EINVAL;
		midr_prefix_from_in_addr(&self_id, g1->router_id);
		desired_unique = midr_nds_locator_unique(g1, &self_id,
						   &desired);
	}
	...
	if (want_transport && desired_unique && midr_ctrl_open(g1, &desired)) {
	...
	midr_pm_init(g1);
	midr_nds_local_node_update(g1);
	midr_nds_report_node(g1, MIDR_ORIGIN_TRANSPORT_UPDATE);
	...
	if (mi->transport_active) {
		...
		for (ALL_LIST_ELEMENTS(mi->session_ledger, node, nnode, ledger)) {
			...
			if (ledger->reason == MIDR_SESSION_PEER_REQ_REPLY) {
				midr_nds_ledger_drop(g1, ledger->transport);
				continue;
			}
			...
			midr_ctrl_connect(g1, &target, ledger->reason,
					  send_nudge);
		}
		midr_nds_manual_sessions_restore(g1);
		midr_pm_on_transport_addr_set(g1);
		...
		if (!mi->shutdown)
			midr_nds_attach_ensure(g1, "transport activated");
	}

	return ret;
}

int midr_nds_transport_configure(struct midr_g1 *g1,
				 const struct ipaddr *transport)
{
	struct midr_nds *mi;
	struct listnode *node;
	...
	if (midr_g1_config_inprocess())
		return 0;
	return midr_nds_transport_reconcile(g1);
}

static int midr_nds_transport_after_cfg(struct midr_g1 *g1)
{
	(void)midr_nds_transport_reconcile(g1);
	return 0;
}

void midr_nds_init(struct midr_g1 *g1)
{
	struct midr_nds *mi;
	...
	/* midrd: session changes arrive via midr_nds_session_status(); the
	 * router-id is fixed on the command line, so there is no rid hook. */
	if (!hooks_registered) {
		hook_register(midr_g1_config_end, midr_nds_transport_after_cfg);
		hook_register(midr_g1_config_end, midr_nds_attach_after_cfg);
		hook_register(midr_g1_config_end, midr_nds_bootstrap_after_cfg);
		hook_register(midr_g1_config_end, midr_nds_remote_view_after_cfg);
		hook_register(midr_g1_config_end, midr_nds_facts_report_after_cfg);
		hooks_registered = true;
	}

	mi = XCALLOC(MTYPE_MIDR_G1, sizeof(*mi));
	mi->g1 = g1;
	...
	if (midr_store_init() == 0) {
		midr_store_seed_load(midr_bootstrap_seed_load_cb, g1);
		event_add_timer(midr_g1_master(), midr_bootstrap_self_boot_cb, g1,
...
void midr_nds_finish(struct midr_g1 *g1)
{
	struct midr_nds *mi;
	...
	XFREE(MTYPE_MIDR_G1, mi);
	g1->midr_nds_info = NULL;
