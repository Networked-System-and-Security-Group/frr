...
extern void midr_nds_facts_init(struct midr_g1 *g1);
extern void midr_nds_facts_finish(struct midr_g1 *g1);
 ...
 */
extern bool midr_nds_facts_node_refresh(struct midr_g1 *g1);
 ...
 */
extern struct midr_context *midr_nds_group2_ctx(struct midr_g1 *g1);
 ...
 */
extern void midr_nds_report_node(struct midr_g1 *g1,
...
midr_nds_facts_link_find(struct midr_g1 *g1, uint32_t remote_node_id,
			 uint64_t link_id);

/* 找不到就建一条（version 初值为 0，首次实际上报前递增为 1）。 */
extern struct midr_nds_fact_link *
midr_nds_facts_link_get(struct midr_g1 *g1, uint32_t remote_node_id,
			uint64_t link_id);
 ...
 */
extern void midr_nds_facts_link_del(struct midr_g1 *g1,
 ...
 */
extern uint64_t midr_nds_facts_link_id(struct midr_g1 *g1,
 ...
 * 该函数头注释。
 */
extern void midr_nds_report_link(struct midr_g1 *g1,
 ...
 */
extern void midr_nds_report_link_withdraw(struct midr_g1 *g1,
					  const struct prefix *remote_node_id);
/* Withdraw every previously reported link while retaining version tombstones. */
extern void midr_nds_facts_withdraw_all_links(struct midr_g1 *g1);
 ...
 */
extern int midr_nds_facts_report_after_cfg(struct midr_g1 *g1);
 ...
 */
extern int midr_nds_facts_report_after_rid(struct midr_g1 *g1, bool withdraw);
...
/* Complete local fact table for midrd resync (midr_topology_provider_register);
 * also read by `show midr group2 snapshot`. */
extern int midr_nds_topology_snapshot_get(struct midr_context *ctx,
					  struct midr_topology_snapshot *snapshot);
extern void midr_nds_topology_snapshot_release(
