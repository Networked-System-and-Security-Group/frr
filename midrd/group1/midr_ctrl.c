...
DEFINE_MTYPE_STATIC(MIDR_G1, MIDR_CTRL_PENDING, "MIDR ctrl pending peer-request");
...
static void midr_ctrl_send_req(struct midr_nds *mi, struct ipaddr dst,
			       uint8_t type, uint32_t target_group);
static void midr_ctrl_enqueue_request(struct midr_g1 *g1, struct ipaddr dst,
				      uint8_t type, uint32_t target_group,
				      const struct midr_ctrl_retx_params *rp);
static void midr_ctrl_send_peer_request(struct midr_g1 *g1,
					const struct midr_node_entry *entry,
					enum midr_session_reason reason);
static void midr_ctrl_drop_pending(struct midr_nds *mi, struct ipaddr dst,
				   uint8_t type);
...
static void midr_ctrl_member_probe_done_cb(struct event *t)
{
	...
	midr_nds_notify_cl(g1, MIDR_TRIGGER_MEMBER_PROBE_DONE);
}
...
static void midr_ctrl_anchor_probe_done_cb(struct event *t)
{
	...
	midr_nds_notify_cl(g1, MIDR_TRIGGER_ANCHOR_PROBE_DONE);
}
...
static bool midr_ctrl_transport_session_up(struct midr_g1 *g1,
...
bool midr_ctrl_encode_locator(uint8_t *wire, const struct ipaddr *locator)
{
...
bool midr_ctrl_decode_locator(const uint8_t *wire, struct ipaddr *locator)
{
	uint16_t afi;
...
static bool midr_ctrl_same_active_family(struct midr_g1 *g1,
					 const struct ipaddr *locator)
{
	struct ipaddr local;
	struct prefix owner = {};
	...
	owner.u.prefix4 = midr_nds_rid_by_transport(g1, *locator);
	return midr_nds_locator_unique(g1, &owner, locator);
}

bool midr_ctrl_encode_request(struct midr_g1 *g1, uint8_t *payload, size_t len,
			      uint8_t type, uint32_t target_group)
{
	struct ipaddr local;

	if (!g1 || !payload || len != MIDR_CTRL_REQUEST_LEN ||
	    !midr_ctrl_is_request_type(type) ||
	    g1->router_id.s_addr == INADDR_ANY ||
	...
	payload[1] = type;
	memcpy(payload + 4, &g1->router_id, sizeof(g1->router_id));
	if (!midr_ctrl_encode_locator(payload + 8, &local))
		return false;
	midr_ctrl_put_u32(payload + 28, (uint32_t)g1->as);
	midr_ctrl_put_u32(payload + 32, target_group);
	return true;
}

static bool midr_ctrl_list_identity_usable(struct midr_g1 *g1,
					 struct in_addr rid,
					 const struct ipaddr *transport);

static bool midr_ctrl_decode_request(struct midr_g1 *g1, const uint8_t *payload,
				     size_t len, struct ipaddr source,
				     struct midr_ctrl_request *request)
{
	if (!g1 || !payload || !request || len != MIDR_CTRL_REQUEST_LEN ||
	    payload[0] != MIDR_CTRL_MSG_VERSION ||
	...
	if (!midr_ipaddr_valid_locator(&source) ||
	    ...
	    !midr_ctrl_same_active_family(g1,
					 &request->requester_transport) ||
	    IPV4_ADDR_SAME(&request->requester_rid, &g1->router_id) ||
	    !midr_ctrl_list_identity_usable(g1, request->requester_rid,
					    &request->requester_transport))
		return false;
	return true;
}
			       ...
			       uint8_t type, uint32_t target_group)
{
	...
	if (!mi || !mi->g1 || mi->ctrl_sock < 0 ||
	    !midr_ctrl_is_udp_type(type) ||
	    !midr_ipaddr_valid_locator(&dst) ||
	    !midr_ctrl_same_active_family(mi->g1, &dst))
		return;
	if (!midr_ctrl_encode_request(mi->g1, payload, sizeof(payload), type,
				      target_group))
		return;
...
bool midr_ctrl_send_datagram(struct midr_g1 *g1, const struct ipaddr *dst,
			     const uint8_t *buf, size_t len)
{
	struct midr_nds *mi;
	union sockunion su;

	if (!g1 || !(mi = g1->midr_nds_info) || mi->ctrl_sock < 0 || !dst ||
	    !midr_ipaddr_valid_locator(dst) ||
	    !midr_ctrl_same_active_family(g1, dst) ||
	    !midr_ipaddr_to_sockunion(dst, &su))
		return false;
	if (sockunion_family(&su) == AF_INET)
		su.sin.sin_port = htons(MIDR_CTRL_UDP_PORT);
	else
		su.sin6.sin6_port = htons(MIDR_CTRL_UDP_PORT);
	if (sendto(mi->ctrl_sock, buf, len, 0, (struct sockaddr *)&su,
		   sockunion_sizeof(&su)) < 0) {
		MIDR_LOG("MIDR ctrl: datagram type %u sendto %pIA failed: %s",
			 len > 1 ? buf[1] : 0, dst, safe_strerror(errno));
		return false;
	}
	return true;
}
 ...
 */
static void midr_ctrl_request_attempt(struct midr_g1 *g1, struct ipaddr dst,
				      uint8_t type, uint32_t target_group)
{
	if (!g1 || !g1->midr_nds_info)
		return;
	if (midr_ctrl_is_udp_type(type))
		midr_ctrl_send_req(g1->midr_nds_info, dst, type, target_group);
	else if (midr_ctrl_is_tcp_request_type(type))
		midr_ctrl_tcp_client_start(g1, dst, type, target_group);
}
				      ...
				      const struct midr_ctrl_retx_params *rp)
{
	...
	midr_ctrl_request_attempt(g1, dst, type, target_group);
	...
	if (!mi->t_ctrl_retx) {
		...
		event_add_timer_msec(midr_g1_master(), midr_ctrl_retx_timer, g1,
				     rp->interval_ms, &mi->t_ctrl_retx);
	}
}

void midr_ctrl_set_retx_budget(struct midr_g1 *g1, struct ipaddr dst,
			       uint8_t type, int retries)
{
	...
	if (!g1 || !g1->midr_nds_info ||
	    !g1->midr_nds_info->ctrl_pending || retries <= 0)
		return;
	mi = g1->midr_nds_info;
	if (type == MIDR_CTRL_ATTACH_REQUEST)
		midr_admission_retx_budget(g1, dst, retries);
...
void midr_ctrl_send_rep_request(struct midr_g1 *g1,
				struct ipaddr bootstrap_transport)
{
	if (!g1 || !g1->midr_nds_info)
		return;
	midr_ctrl_enqueue_request(g1, bootstrap_transport,
...
void midr_ctrl_send_member_request(struct midr_g1 *g1, struct ipaddr rep_transport,
				   uint32_t group_id)
{
	if (!g1 || !g1->midr_nds_info)
		return;
	midr_ctrl_enqueue_request(g1, rep_transport, MIDR_CTRL_MEMBER_LIST_REQ,
...
void midr_ctrl_send_bootstrap_list_request(struct midr_g1 *g1, struct ipaddr dst)
{
	if (!g1 || !g1->midr_nds_info)
		return;
	midr_ctrl_enqueue_request(g1, dst, MIDR_CTRL_BOOTSTRAP_LIST_REQ, 0,
				  &midr_ctrl_retx_default);
 ...
 */
void midr_ctrl_send_announce(struct midr_g1 *g1, struct ipaddr dst)
{
	if (!g1 || !g1->midr_nds_info)
		return;
	midr_ctrl_send_req(g1->midr_nds_info, dst, MIDR_CTRL_ANNOUNCE, 0);
 ...
 */
static uint8_t midr_ctrl_pending_peer_req_type(struct midr_nds *mi,
					       struct ipaddr dst)
{
	struct listnode *node;
	struct midr_ctrl_pending *p;
					...
					enum midr_session_reason reason)
{
	...
	if (!entry->has_transport_addr ||
	    !midr_ctrl_same_active_family(g1, &entry->transport_addr))
		return;
	...
	midr_ctrl_enqueue_request(g1, entry->transport_addr, type,
				  mi->local_group_id, &midr_ctrl_retx_default);
	if (reason == MIDR_SESSION_ATTACH) {
		int budget = midr_admission_get_retx_budget(g1, entry->transport_addr);
		if (budget > 0)
			midr_ctrl_set_retx_budget(g1, entry->transport_addr, type, budget);
	}
 ...
 */
static void midr_ctrl_send_peer_reject(struct midr_g1 *g1, struct ipaddr dst)
{
	struct midr_nds *mi = g1->midr_nds_info;
 ...
 * 日志却报"重传 5 次无回应"）。
 */
static void midr_ctrl_drop_half_peer(struct midr_g1 *g1, struct ipaddr transport,
				     const char *why)
{
	union sockunion su;
	struct midr_g1_peer *peer;
	...
	midr_nds_cleanup_by_transport(g1, transport,
...
static void midr_ctrl_retx_timer(struct event *t)
{
	...
	for (ALL_LIST_ELEMENTS(mi->ctrl_pending, node, nnode, p)) {
		...
		midr_ctrl_request_attempt(g1, p->target_transport, p->type,
					  p->target_group);
	}
 ...
 */
static struct stream *midr_ctrl_build_rep_list(struct midr_g1 *g1,
					       struct ipaddr dst)
{
	struct midr_nds *mi = g1->midr_nds_info;
	...
	midr_rep_candidates(g1, derived);
	...
	for (ALL_LIST_ELEMENTS_RO(mi->rep_dir, node, r)) {
		...
		if (!midr_ctrl_same_active_family(g1, &r->rep_transport) ||
	...
	for (ALL_LIST_ELEMENTS_RO(derived, node, ne)) {
		...
		if (ne->node_id.family != AF_INET ||
		    ...
		    !midr_ctrl_same_active_family(g1, &ne->transport_addr))
			continue;
		if (midr_ctrl_list_has_identity(
			    s, count, MIDR_CTRL_REP_ITEM_LEN, 4, 28,
 ...
 */
static struct stream *midr_ctrl_build_member_list(struct midr_g1 *g1,
						  struct ipaddr dst,
						  uint32_t group_id)
{
	struct stream *s;
	...
	members = list_new();
	midr_group_members(g1, group_id, members);
	...
	/* Ourselves (the representative) first. */
	{
		...
		if (g1->router_id.s_addr != INADDR_ANY &&
		    midr_nds_local_transport_get(g1, &local) &&
		    midr_ctrl_stream_put_member_item(
			    s, g1->router_id, &local, g1->as, group_id))
			count++;
	}
 ...
 */
static struct stream *midr_ctrl_build_bootstrap_list(struct midr_g1 *g1,
						     struct ipaddr dst)
{
	struct midr_nds *mi = g1->midr_nds_info;
	struct stream *s;
	...
	for (ALL_LIST_ELEMENTS_RO(mi->bootstrap_list, node, b)) {
		...
		if (mi->transport_active && midr_ipaddr_same(
			...
			continue;
		if (!midr_ctrl_transport_session_up(g1, b->transport)) {
			n_skip++;
			continue;
		}
		if (b->rid.s_addr == INADDR_ANY ||
		    !midr_ctrl_same_active_family(g1, &b->transport))
			continue;
		if (midr_ctrl_list_has_identity(
					   ...
					   const struct ipaddr *transport)
{
	...
	if (!g1 || !g1->midr_nds_info ||
	    !g1->midr_nds_info->global_view || !transport ||
	...
	if (IPV4_ADDR_SAME(&rid, &g1->router_id))
		return midr_ipaddr_same(&local, transport) &&
		       midr_nds_locator_unique(g1, &key.node_id, transport);
	...
	return midr_nds_locator_unique(g1, &key.node_id, transport);
}
...
static bool midr_ctrl_bootstrap_locator_binding_usable(
	const struct midr_nds *mi, struct in_addr rid,
	const struct ipaddr *transport)
{
	struct listnode *node;
...
static void midr_ctrl_recv_bootstrap_list(struct midr_g1 *g1, const uint8_t *buf,
					  ssize_t n, struct ipaddr src)
{
	...
	if (n < 0 || !midr_ctrl_decode_list_header(
	    ...
	    !midr_ctrl_same_active_family(g1, &src)) {
	...
	midr_nds_bootstrap_list_begin(g1);
	for (i = 0; i < count; i++)
		midr_nds_bootstrap_learn(g1, items[i].transport, items[i].asn,
					 items[i].rid);
	XFREE(MTYPE_TMP, identities);
	XFREE(MTYPE_TMP, items);
	midr_nds_on_bootstrap_list(g1, src, count);
	return;
...
static void midr_ctrl_recv_rep_list(struct midr_g1 *g1, const uint8_t *buf,
				    ssize_t n, struct ipaddr src)
{
	struct midr_nds *mi = g1->midr_nds_info;
 ...
 */
static void midr_ctrl_recv_member_list(struct midr_g1 *g1, const uint8_t *buf,
				       ssize_t n, struct ipaddr src)
{
	struct midr_nds *mi = g1->midr_nds_info;
	...
	for (i = 0; i < count; i++) {
		...
		if (IPV4_ADDR_SAME(&items[i].rid, &g1->router_id))
			continue; /* 跳过描述自己的条目 */

		if (is_join_candidate) {
			midr_nds_learn_member(
				g1, items[i].rid, items[i].asn,
	...
	if (is_join_candidate) {
		...
		event_add_timer(midr_g1_master(), midr_ctrl_member_probe_done_cb,
				g1, MIDR_JOIN_PROBE_WAIT_SECS,
		...
		event_add_timer(midr_g1_master(), midr_ctrl_anchor_probe_done_cb,
				g1, MIDR_JOIN_PROBE_WAIT_SECS,
 ...
 */
struct stream *midr_ctrl_on_tcp_request(struct midr_g1 *g1, const uint8_t *payload,
					size_t len, struct ipaddr remote)
{
	...
	if (!g1 || !g1->midr_nds_info)
		return NULL;
	mi = g1->midr_nds_info;
	if (!midr_ctrl_decode_request(g1, payload, len, remote, &request)) {
	...
	case MIDR_CTRL_REP_LIST_REQ:
		...
		MIDR_FLOW_LOG("MIDR ctrl: REP_LIST_REQ from %pIA — replying with rep directory",
			      &request.requester_transport);
		return midr_ctrl_build_rep_list(g1, remote);
	case MIDR_CTRL_MEMBER_LIST_REQ: {
	...
	case MIDR_CTRL_BOOTSTRAP_LIST_REQ:
		...
		MIDR_FLOW_LOG("MIDR ctrl: BOOTSTRAP_LIST_REQ from %pIA — replying with live bootstrap list",
			      &request.requester_transport);
		return midr_ctrl_build_bootstrap_list(g1, remote);
	default:
 ...
 * + 推进 join。
 */
void midr_ctrl_on_tcp_response(struct midr_g1 *g1, uint8_t req_type,
			       const uint8_t *payload, size_t len,
			       struct ipaddr src)
{
	uint8_t response_type;

	if (!g1 || !g1->midr_nds_info || !payload)
		return;
	...
	if (g1->midr_nds_info && g1->midr_nds_info->shutdown) {
	...
	case MIDR_CTRL_REP_LIST_RESP:
		...
		midr_ctrl_recv_rep_list(g1, payload, (ssize_t)len, src);
		break;
	case MIDR_CTRL_MEMBER_LIST_RESP:
		...
		midr_ctrl_recv_member_list(g1, payload, (ssize_t)len, src);
		break;
	case MIDR_CTRL_BOOTSTRAP_LIST_RESP:
		...
		midr_ctrl_recv_bootstrap_list(g1, payload, (ssize_t)len, src);
		break;
	default:
...
static void midr_ctrl_udp_recv(struct event *t)
{
	struct midr_g1 *g1 = EVENT_ARG(t);
	struct midr_nds *mi = g1->midr_nds_info;
	/* Room for the largest UDP frame (NODE_ADV) plus one byte to detect
	 * oversized datagrams. */
	uint8_t buf[MIDR_CTRL_NODE_ADV_LEN + 1];
	struct midr_ctrl_request request;
	...
	event_add_read(midr_g1_master(), midr_ctrl_udp_recv, g1, mi->ctrl_sock,
		       &mi->t_ctrl_read);
	...
	if (!midr_sockunion_to_ipaddr(&source_su, &source)) {
		MIDR_LOG("MIDR ctrl: UDP datagram from an unusable source — dropping");
		return;
	}
	/* midrd: node directory frames are relayed hop by hop, so their source
	 * is the neighbor that forwarded them, not the advertised node. */
	if (n >= 2 && buf[1] == MIDR_CTRL_NODE_ADV) {
		if (!mi->shutdown)
			midr_nodedir_receive(g1, &source, buf, (size_t)n);
		return;
	}
	if (!midr_ctrl_decode_request(g1, buf, (size_t)n, source, &request)) {
	...
	case MIDR_CTRL_ATTACH_REQUEST: {
		...
		if (!is_attach && (mi->local_capabilities & MIDR_CAP_BOOTSTRAP)) {
			...
			if (rentry &&
				...
				midr_ctrl_send_peer_reject(
					g1, request.requester_transport);
				return;
			}
		}
		...
		midr_admission_remote_seen(g1, req.transport_addr);
		if (midr_ctrl_connect_received(g1, &req, reason, is_attach) == MIDR_ADMISSION_BLOCKED)
			midr_ctrl_send_peer_reject(g1, req.transport_addr);
	...
	case MIDR_CTRL_PEER_REJECT: {
		...
		midr_ctrl_drop_half_peer(g1, request.requester_transport,
		...
		if (req_type == MIDR_CTRL_ATTACH_REQUEST) {
			...
			batch = midr_nds_attach_mark_failed(
				g1, request.requester_transport);
			midr_nds_attach_pick_from(g1, batch);
		}
		break;
	}
	case MIDR_CTRL_ANNOUNCE:
...
void midr_ctrl_init(struct midr_g1 *g1)
{
	struct midr_nds *mi;
	...
	midr_ctrl_tcp_init(g1);
}

void midr_ctrl_close(struct midr_g1 *g1)
{
	struct midr_nds *mi;
	struct listnode *node, *nnode;
	struct midr_ctrl_pending *p;
...
void midr_ctrl_forget_target(struct midr_g1 *g1, struct ipaddr transport)
{
	struct midr_nds *mi;
	struct listnode *node, *nnode;
	...
	midr_ctrl_tcp_cancel_target(g1, transport);
	if (!midr_ipaddr_to_sockunion(&transport, &su))
		return;
	peer = midr_g1_peer_lookup_su(g1, &su);
	if (peer && midr_nds_peer_is_overlay(peer))
...
bool midr_ctrl_open(struct midr_g1 *g1, const struct ipaddr *local)
{
	struct midr_nds *mi;
	union sockunion su;
	int sock;
	...
	midr_ctrl_close(g1);
	if (!midr_ctrl_tcp_open(g1, local))
		return false;
	...
	mi->ctrl_sock = sock;
	event_add_read(midr_g1_master(), midr_ctrl_udp_recv, g1, sock,
		       &mi->t_ctrl_read);
...
void midr_ctrl_finish(struct midr_g1 *g1)
{
	struct midr_nds *mi;
...
/*
 * 会话归属判据（⑦）：bgpd 里要区分 MIDR 自建会话和运维原生会话；midrd 的会话表
 * 只装第一组请求的会话，表里有即是 MIDR 自己的。
 */
bool midr_nds_peer_is_overlay(struct midr_g1_peer *peer)
{
	return peer != NULL;
}

static enum midr_admission_result midr_ctrl_connect_internal(struct midr_g1 *g1, const struct midr_node_entry *entry,
		       enum midr_session_reason reason, bool send_nudge, bool received, bool attach_request)
{
	...
	if (!midr_nds_locator_unique(g1, &entry->node_id,
				      &entry->transport_addr)) {
	 ...
	 */
	if ((has_remote_rid &&
	     IPV4_ADDR_SAME(&entry->node_id.u.prefix4, &g1->router_id)) ||
	    (mi->transport_active &&
	...
	if (!peer || !peer->established) {
		enum midr_admission_result admission = midr_admission_gate(
			g1, entry, reason, send_nudge, received, attach_request);
	 ...
	 */
	midr_nds_ledger_note(g1, entry->transport_addr, reason, remote_rid,
			     asn, entry->group_id);
	...
	if (reason == MIDR_SESSION_SAME_GROUP) {
		...
		midr_nds_adopt_group_peer(g1, remote_rid, asn, transport,
					  entry->group_id);
		midr_mark_topology(g1, entry);
		if (adjacency_changed)
			midr_admission_committed(g1);
	}
	 ...
	 */
	if (send_nudge)
		midr_ctrl_send_peer_request(g1, entry, reason);
	 ...
	 */
	{
		struct midr_g1_peer *occupant = midr_g1_peer_lookup_su(g1, &su);

		if (occupant) {
			if (midr_nds_peer_is_overlay(occupant)) {
				/* An admission-held session is requested again now. */
				midr_g1_peer_start(occupant);
			} else {
	...
	/* Create the native MIDR session through midrd (midr-session.h). */
	peer = midr_g1_peer_create(g1, &entry->transport_addr, remote_rid, asn);
	if (!peer) {
		zlog_warn("MIDR ctrl: session request for %pFX %pIA failed",
...
enum midr_admission_result midr_ctrl_connect(struct midr_g1 *g1,
	const struct midr_node_entry *entry, enum midr_session_reason reason,
	bool send_nudge)
{
	return midr_ctrl_connect_internal(g1, entry, reason, send_nudge, false, false);
}

enum midr_admission_result midr_ctrl_connect_received(struct midr_g1 *g1,
	const struct midr_node_entry *entry, enum midr_session_reason reason,
	bool attach_request)
{
	return midr_ctrl_connect_internal(g1, entry, reason, false, true, attach_request);
}

void midr_ctrl_reject_admission(struct midr_g1 *g1, struct ipaddr transport)
{
	midr_ctrl_send_peer_reject(g1, transport);
}

static void midr_try_disconnect(struct midr_g1 *g1,
				const struct midr_node_entry *entry,
				bool force)
{
	union sockunion su;
	...
	if (!force) {
		const struct midr_session_ledger_entry *led =
			midr_nds_ledger_lookup(g1, transport);

		if ((led && led->reason == MIDR_SESSION_MANUAL) ||
		    midr_admission_is_manual(g1, transport)) {
	...
	struct midr_g1_peer *peer = midr_g1_peer_lookup_su(g1, &su);
	if (!peer)
		return;
...
void midr_ctrl_on_node_remove(struct midr_g1 *g1, struct midr_node_entry *entry)
{
	/* 自动路径（对端 withdraw / locator 换代 / 换组清理）：α 豁免照旧生效。 */
	midr_try_disconnect(g1, entry, false);
}
 ...
 */
void midr_ctrl_detach_transport(struct midr_g1 *g1, struct ipaddr transport,
				...
				enum midr_stop_reason reason)
{
	...
	midr_try_disconnect(g1, &e, force);

	/* 拆完会话还要收拾节点表那半边（停探 + 撤链路上报 + 清 link_entry +
	 * 清 is_adjacent）——临时条目碰不到表里的真条目，不补就留下"标着邻居、
	 * 边却没了"的幻影。对端是引导时反查落空、天然 no-op。 */
	midr_nds_cleanup_by_transport(g1, transport, reason);
}

void midr_mark_topology(struct midr_g1 *g1, const struct midr_node_entry *entry)
{
	(void)g1;
...
int midr_ctrl_connect_group(struct midr_g1 *g1, uint32_t group_id,
			    enum midr_session_reason reason)
{
	struct midr_node_entry *entry;
	int count = 0;
	...
	frr_each (midr_node_hash, &g1->midr_nds_info->global_view->nodes, entry) {
		...
		enum midr_admission_result admission =
			midr_ctrl_connect(g1, entry, reason, true);
