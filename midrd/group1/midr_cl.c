...
DEFINE_MTYPE_STATIC(MIDR_G1, MIDR_NODE_EVIDENCE, "MIDR node evidence");
DEFINE_MTYPE_STATIC(MIDR_G1, MIDR_REP_IDENTITY, "MIDR representative identity");
 ...
 */
static void cl_handle_rep_probe_done(struct midr_g1 *g1,
				     const struct midr_global_view *gv)
{
	...
	for (chosen = 0; chosen < nrank; chosen++) {
		if (!midr_admission_candidate_blocked(g1,
	...
	if (chosen == nrank) {
		...
		MIDR_FLOW_LOG(
			...
			new_gid);
		midr_nds_on_cluster_decision(g1, &d);
	} else {
 ...
 */
static size_t cl_count_good_member_links(const struct midr_g1 *g1,
					 const struct midr_global_view *gv,
	...
	for (ALL_LIST_ELEMENTS_RO(nodes, n, entry)) {
		...
		if (cl_link_is_good_for_join(link) &&
		    (!entry->has_transport_addr ||
		     !midr_admission_candidate_blocked(g1, entry->transport_addr))) {
			good++;
		} else {
			/* 收集未达标链路数据用于调试日志 */
 ...
 */
static void cl_handle_member_probe_done(struct midr_g1 *g1,
					const struct midr_global_view *gv)
{
	struct midr_nds *mi = g1->midr_nds_info;
	...
	good_links = cl_count_good_member_links(g1, gv, target_gid,
 ...
 */
static void cl_select_anchor_candidates(struct midr_g1 *g1, const struct midr_global_view *gv,
					uint32_t target_group_id,
					struct list *out)
{
	struct list *nodes;
 ...
 */
static void cl_handle_anchor_probe_done(struct midr_g1 *g1,
					const struct midr_global_view *gv)
{
	struct midr_nds *mi = g1->midr_nds_info;
	struct midr_cluster_decision d = {};
	...
	cl_select_anchor_candidates(g1, gv, mi->anchor_group_id[0], d.evidence);
	cl_select_anchor_candidates(g1, gv, mi->anchor_group_id[1], d.evidence);
	{
 ...
 */
static void cl_handle_periodic_sync(struct midr_g1 *g1,
				    const struct midr_global_view *gv)
{
	struct midr_nds *mi = g1->midr_nds_info;
	uint32_t worst_rtt = 0;
	...
	good_links = cl_count_good_member_links(g1, gv, mi->local_group_id,
						&worst_rtt, &worst_loss,
						&total_adjacent);
 ...
 */
static void midr_cl_on_global_view(struct midr_g1 *g1,
				   ...
				   const struct midr_global_view *gv)
{
	...
	case MIDR_TRIGGER_REP_PROBE_DONE:
		...
		cl_handle_rep_probe_done(g1, gv);
		break;
	...
	case MIDR_TRIGGER_MEMBER_PROBE_DONE:
		...
		cl_handle_member_probe_done(g1, gv);
		break;
	...
	case MIDR_TRIGGER_ANCHOR_PROBE_DONE:
		...
		cl_handle_anchor_probe_done(g1, gv);
		break;
	...
	case MIDR_TRIGGER_ADMISSION_CHANGE: {
		uint32_t groups[2];
		size_t count = midr_admission_anchor_groups(g1, groups, array_size(groups));
		struct midr_cluster_decision d = {};
		...
		for (size_t i = 0; i < count; i++)
			cl_select_anchor_candidates(g1, gv, groups[i], d.evidence);
...
void midr_cl_register_callback(struct midr_g1 *g1, midr_global_view_cb cb)
{
	if (g1 && g1->midr_nds_info)
		g1->midr_nds_info->cl_callback = cb;
}

void midr_cl_init(struct midr_g1 *g1)
{
	midr_cl_register_callback(g1, midr_cl_on_global_view);
}
