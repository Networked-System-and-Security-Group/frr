 ...
 * that supplies the measured ASN sequence.  (The bgpd copy also checks a BGP
 * AS_PATH; midrd has no BGP RIB, so that helper is not carried over.)
 */
...
/* Private ASN ranges (RFC 6996), as in bgpd/bgp_aspath.h. */
#define MIDR_PRIVATE_AS_MIN 64512U
#define MIDR_PRIVATE_AS_MAX 65535U
#define MIDR_PRIVATE_AS4_MIN 4200000000U
#define MIDR_PRIVATE_AS4_MAX 4294967294U
#define MIDR_AS_IS_PRIVATE(asn)                                                \
	(((asn) >= MIDR_PRIVATE_AS_MIN && (asn) <= MIDR_PRIVATE_AS_MAX) ||     \
	 ((asn) >= MIDR_PRIVATE_AS4_MIN && (asn) <= MIDR_PRIVATE_AS4_MAX))
