...
extern int midr_pm_add_target(struct midr_g1 *g1, const struct prefix *node_id,
			      enum midr_node_source source,
			      uint32_t capabilities);

/* I-2: NDS -> PM, stop probing a node */
extern int midr_pm_remove_target(struct midr_g1 *g1, const struct prefix *node_id,
 ...
 */
extern int midr_pm_remove_all_targets(struct midr_g1 *g1,
				      enum midr_stop_reason reason);
...
extern void midr_pm_on_transport_addr_set(struct midr_g1 *g1);

/* Called after the local transport address is removed. */
extern void midr_pm_on_transport_addr_unset(struct midr_g1 *g1);

/* Module lifecycle: open/close PM socket and arm/cancel probe timers.
 * Called from bgp_midr_nds_init() / bgp_midr_nds_finish(). */
extern void midr_pm_init(struct midr_g1 *g1);
extern void midr_pm_finish(struct midr_g1 *g1);
