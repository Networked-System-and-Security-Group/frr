 ...
 * Evaluates peering policy and drives peer_remote_as() / midr_g1_peer_delete()
...
enum midr_ctrl_msg_type {
	...
	/*
	 * midrd 节点目录通告（UDP，逐跳转发）：bgpd 里第二组替我们把每个节点的
	 * 群号、locator、角色位泛洪给全网；midrd 的 Membership 只带群号，这份目录
	 * 改由第一组自己沿 overlay 会话泛洪，格式见 midr_nodedir.c。
	 */
	MIDR_CTRL_NODE_ADV = 11,
};

#define MIDR_CTRL_NODE_ADV_LEN 48U

/* Version-4 locator codec (MIDR_CTRL_LOCATOR_LEN bytes). */
extern bool midr_ctrl_encode_locator(uint8_t *wire,
				     const struct ipaddr *locator);
extern bool midr_ctrl_decode_locator(const uint8_t *wire,
				     struct ipaddr *locator);
/* Send one prepared control datagram to dst's control port. */
extern bool midr_ctrl_send_datagram(struct midr_g1 *g1,
...
extern void midr_ctrl_init(struct midr_g1 *g1);
extern enum midr_admission_result midr_ctrl_connect_received(struct midr_g1 *g1,
	const struct midr_node_entry *entry, enum midr_session_reason reason,
	bool attach_request);
extern void midr_ctrl_reject_admission(struct midr_g1 *g1, struct ipaddr transport);
extern void midr_ctrl_finish(struct midr_g1 *g1);
/* Atomically open/close UDP and TCP channels on one exact local locator. */
extern bool midr_ctrl_open(struct midr_g1 *g1, const struct ipaddr *local);
extern void midr_ctrl_close(struct midr_g1 *g1);
 ...
 * responsibility. */
extern void midr_ctrl_forget_target(struct midr_g1 *g1,
 ...
 * nudge"这类错误静默化。
 */
extern enum midr_admission_result midr_ctrl_connect(struct midr_g1 *g1,
 ...
 */
extern bool midr_nds_peer_is_overlay(struct midr_g1_peer *peer);
 ...
 * one exists.
 */
extern void midr_ctrl_on_node_remove(struct midr_g1 *g1,
 ...
 * SESSION_DOWN）。 */
extern void midr_ctrl_detach_transport(struct midr_g1 *g1,
				       struct ipaddr transport,
 ...
 */
extern int midr_ctrl_connect_group(struct midr_g1 *g1, uint32_t group_id,
 ...
 */
extern void midr_ctrl_send_rep_request(struct midr_g1 *g1,
				       struct ipaddr bootstrap_transport);
extern void midr_ctrl_send_member_request(struct midr_g1 *g1,
 ...
 */
extern void midr_ctrl_send_bootstrap_list_request(struct midr_g1 *g1,
						  struct ipaddr dst);
 ...
 */
extern void midr_ctrl_set_retx_budget(struct midr_g1 *g1,
				      struct ipaddr dst,
 ...
 */
extern void midr_ctrl_send_announce(struct midr_g1 *g1,
				    struct ipaddr dst);
 ...
 */
extern void midr_mark_topology(struct midr_g1 *g1,
			       const struct midr_node_entry *entry);
 ...
 */
extern bool midr_ctrl_encode_request(struct midr_g1 *g1, uint8_t *payload,
 ...
 */
extern struct stream *midr_ctrl_on_tcp_request(struct midr_g1 *g1,
 ...
 */
extern void midr_ctrl_on_tcp_response(struct midr_g1 *g1, uint8_t req_type,
...
extern void midr_ctrl_tcp_init(struct midr_g1 *g1);
extern void midr_ctrl_tcp_finish(struct midr_g1 *g1);
/* Open/close the exact-address listener and all current short connections. */
extern bool midr_ctrl_tcp_open(struct midr_g1 *g1,
			       const struct ipaddr *local);
extern void midr_ctrl_tcp_close(struct midr_g1 *g1);
extern void midr_ctrl_tcp_cancel_target(struct midr_g1 *g1,
					struct ipaddr transport);
 ...
 */
extern void midr_ctrl_tcp_client_start(struct midr_g1 *g1,
				       struct ipaddr dst,
				       uint8_t type, uint32_t target_group);

/* 重试定时器查询: 到 dst 的 type 请求是否已有在途 TCP 连接 (在途则本轮不减
 * retries_left，避免虚耗重试数)。 */
extern bool midr_ctrl_tcp_client_inflight(struct midr_nds *mi,
					  struct ipaddr dst,
					  uint8_t type);
