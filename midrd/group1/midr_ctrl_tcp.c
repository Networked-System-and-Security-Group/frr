...
DEFINE_MTYPE_STATIC(MIDR_G1, MIDR_CTRL_TCP_CONN, "MIDR ctrl TCP connection");
...
midr_ctrl_tcp_conn_new(struct midr_g1 *g1, int fd, enum midr_ctrl_conn_role role)
{
	...
	event_add_timer(midr_g1_master(), midr_ctrl_tcp_timeout, conn,
...
{
	struct midr_nds *mi = conn->g1->midr_nds_info;
...
static void midr_ctrl_tcp_start_send(struct midr_ctrl_tcp_conn *conn)
{
	...
	event_add_write(midr_g1_master(), midr_ctrl_tcp_write, conn, conn->fd,
			&conn->t_write);
}

static void midr_ctrl_tcp_want_read(struct midr_ctrl_tcp_conn *conn)
{
	event_add_read(midr_g1_master(), midr_ctrl_tcp_read, conn, conn->fd,
		       &conn->t_read);
}
...
static void midr_ctrl_tcp_write(struct event *t)
{
	...
	if (n < 0) {
		if (ERRNO_IO_RETRY(errno)) {
			event_add_write(midr_g1_master(), midr_ctrl_tcp_write, conn,
...
static void midr_ctrl_tcp_read(struct event *t)
{
	...
	if (conn->role == MIDR_CTRL_CONN_SERVER) {
		...
		resp = midr_ctrl_on_tcp_request(g1, payload, plen,
						conn->remote);
		if (!resp) {
		...
		midr_ctrl_on_tcp_response(g1, conn->type, payload, plen,
					  conn->remote);
		midr_ctrl_tcp_conn_close(conn);
	}
}
...
static void midr_ctrl_tcp_send_request(struct midr_ctrl_tcp_conn *conn)
{
	...
	if (!midr_ctrl_encode_request(conn->g1, payload, sizeof(payload),
...
void midr_ctrl_tcp_client_start(struct midr_g1 *g1, struct ipaddr dst,
				uint8_t type, uint32_t target_group)
{
	...
	owner.u.prefix4 = midr_nds_rid_by_transport(g1, dst);
	if (!midr_nds_locator_unique(g1, &owner, &dst))
		return;
	...
	case connect_success:
		conn = midr_ctrl_tcp_conn_new(g1, fd, MIDR_CTRL_CONN_CLIENT);
		conn->remote = dst;
		conn->type = type;
	...
	case connect_in_progress:
		...
		event_add_read(midr_g1_master(), midr_ctrl_tcp_connect_check, conn,
			       fd, &conn->t_read);
		event_add_write(midr_g1_master(), midr_ctrl_tcp_connect_check, conn,
				fd, &conn->t_write);
		return;
	}
}

bool midr_ctrl_tcp_client_inflight(struct midr_nds *mi, struct ipaddr dst,
				   uint8_t type)
{
	struct listnode *node;
	struct midr_ctrl_tcp_conn *c;
...
void midr_ctrl_tcp_cancel_target(struct midr_g1 *g1, struct ipaddr transport)
{
	struct midr_nds *mi;
...
static void midr_ctrl_tcp_accept(struct event *t)
{
	...
	event_add_read(midr_g1_master(), midr_ctrl_tcp_accept, g1, mi->ctrl_tcp_lsock,
	...
	conn = midr_ctrl_tcp_conn_new(g1, client_fd, MIDR_CTRL_CONN_SERVER);
	(void)midr_sockunion_to_ipaddr(&su, &conn->remote);
	conn->state = MIDR_CTRL_CONN_READING;
	midr_ctrl_tcp_want_read(conn);
}

bool midr_ctrl_tcp_open(struct midr_g1 *g1, const struct ipaddr *local)
{
	struct midr_nds *mi;
	union sockunion su;
	int sock;
	...
	event_add_read(midr_g1_master(), midr_ctrl_tcp_accept, g1, sock,
		       &mi->t_ctrl_tcp_accept);
...
void midr_ctrl_tcp_init(struct midr_g1 *g1)
{
	struct midr_nds *mi;
...
void midr_ctrl_tcp_close(struct midr_g1 *g1)
{
	struct midr_nds *mi;
	struct listnode *node;
...
void midr_ctrl_tcp_finish(struct midr_g1 *g1)
{
	struct midr_nds *mi;
