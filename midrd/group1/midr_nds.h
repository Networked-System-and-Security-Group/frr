...
#include "midrd/group1/midr_addr.h"
#include "midrd/group1/midr_pm.h"

/* bgpd looked sessions up by socket address; keep that call shape. */
static inline struct midr_g1_peer *
midr_g1_peer_lookup_su(struct midr_g1 *g1, const union sockunion *su)
{
	struct ipaddr transport;

	if (!midr_sockunion_to_ipaddr(su, &transport))
		return NULL;
	return midr_g1_peer_lookup(g1, &transport);
}

/*
 * MIDR 日志两件事，别混：**级别**决定"够不够格写出去"，**频道**决定"这类啰嗦
 * 话现在想不想听"。频道（`debug midr [discovery]`）**只管 debug 那一档**
...
typedef void (*midr_global_view_cb)(struct midr_g1 *g1,
...
struct midr_nds {
	...
	struct midr_g1 *g1; /* back-pointer */
...
extern void midr_nds_init(struct midr_g1 *g1);
extern void midr_nds_finish(struct midr_g1 *g1);
...
extern void midr_nds_node_react(struct midr_g1 *g1, struct midr_node_entry *entry,
...
extern void midr_nds_learn_member(struct midr_g1 *g1, struct in_addr rid, as_t asn,
				  struct ipaddr transport,
				  uint32_t group_id);

/* 同上，但不标邻居——锚点候选是跨群评估节点，不是本群成员。 */
extern void midr_nds_learn_anchor_candidate(struct midr_g1 *g1,
...
extern void midr_nds_adopt_group_peer(struct midr_g1 *g1, struct in_addr rid,
				      as_t asn,
				      struct ipaddr transport,
				      uint32_t group_id);
...
extern void midr_nds_cleanup_by_transport(struct midr_g1 *g1,
					  struct ipaddr transport,
 ...
 */
extern void midr_nds_learn_requester(struct midr_g1 *g1, struct in_addr rid,
				     as_t asn,
				     struct ipaddr transport);

/* Refresh the local self-entry before publishing the local Node fact. */
extern void midr_nds_local_node_update(struct midr_g1 *g1);
...
extern void midr_nds_set_capability(struct midr_g1 *g1, uint32_t new_caps);
extern void midr_nds_set_group_id(struct midr_g1 *g1, uint32_t new_gid);
 ...
 */
extern void midr_nds_anchor_ctx_clear(struct midr_g1 *g1);
 ...
 */
extern unsigned int midr_nds_shutdown_enter(struct midr_g1 *g1);
extern bool midr_nds_shutdown_exit(struct midr_g1 *g1);
...
extern void midr_nds_on_link_update(struct midr_g1 *g1,
...
extern void midr_nds_notify_cl(struct midr_g1 *g1,
			       enum midr_trigger_type trigger);
...
extern struct midr_g1_peer *midr_node_established_peer(struct midr_g1 *g1,
					      const struct prefix *node_id);

/* I-7: CL -> NDS, apply a clustering decision */
extern void midr_nds_on_cluster_decision(
	struct midr_g1 *g1, const struct midr_cluster_decision *decision);

/* Refresh the local Node fact after a group-id change. */
extern void midr_originate_group_update(struct midr_g1 *g1, uint32_t new_group_id,
					uint32_t old_group_id);
 ...
 */
extern void midr_join_via_bootstrap(struct midr_g1 *g1, const union sockunion *su,
 ...
 * 行为一致）。
 */
extern void midr_join_bootstrap_failed(struct midr_g1 *g1,
				       struct ipaddr failed);

/* `no midr bootstrap <locator>`：删指定候选（true=找到并删除）；正在尝试的
 * 被删则顺移到下一候选。 */
extern bool midr_bootstrap_list_del(struct midr_g1 *g1,
				    struct ipaddr addr);

/* `no midr bootstrap`（无参）：清空候选清单并作废在途加入意图（与手动换组
 * 同款中止语义，批注点 C 已拍板）。 */
extern void midr_bootstrap_clear(struct midr_g1 *g1);
 ...
 */
extern void midr_nds_bootstrap_list_fetch(struct midr_g1 *g1);
 ...
 */
extern void midr_nds_bootstrap_learn(struct midr_g1 *g1,
 ...
 *   刚学到的一起清了）。
 */
extern void midr_nds_bootstrap_list_begin(struct midr_g1 *g1);

/* 一份名单收完后的汇总回调（src = 应答方，count = 条目数）。批 5 在此接
 * "哈希顺次取 K 台挂靠"。 */
extern void midr_nds_on_bootstrap_list(struct midr_g1 *g1,
				       struct ipaddr src,
				       unsigned int count);
 ...
 */
extern void midr_nds_attach_pick(struct midr_g1 *g1);

/* 同上，但指定从哪一批开始——死心重挑专用（A-3），见 enum midr_attach_batch。 */
extern void midr_nds_attach_pick_from(struct midr_g1 *g1,
...
midr_nds_attach_mark_failed(struct midr_g1 *g1,
			    struct ipaddr transport);
 ...
 */
extern bool midr_nds_is_bootstrap(struct midr_g1 *g1);
 ...
 */
extern bool midr_nds_link_is_backbone(struct midr_g1 *g1,
 ...
 */
extern void midr_nds_bootstrap_enforce(struct midr_g1 *g1);

/* 拆掉全部挂靠边（卸任清位时调用）。只认台账 reason == ATTACH 的条目，
 * 同群/骨干/锚点/手配边一概不碰。 */
extern void midr_nds_attach_detach_all(struct midr_g1 *g1);
 ...
 */
extern void midr_nds_attach_on_session_down(struct midr_g1 *g1,
					    struct ipaddr transport);
 ...
 */
extern void midr_nds_bootstrap_list_failed(struct midr_g1 *g1,
					   struct ipaddr failed);
 ...
 */
extern void midr_join_on_rep_list(struct midr_g1 *g1);
 ...
 */
extern void midr_group_members(struct midr_g1 *g1, uint32_t group_id,
			       struct list *out);
 ...
 */
extern void midr_rep_candidates(struct midr_g1 *g1, struct list *out);

/* Learned group-representative directory.  Control v4 requires nonzero
 * rep_rid and a valid IPv4/IPv6 locator for every entry. */
extern void midr_rep_dir_add(struct midr_g1 *g1, uint32_t group_id,
...
extern struct in_addr midr_nds_bootstrap_rid_by_transport(struct midr_g1 *g1,
							  struct ipaddr transport);

/* 按 transport 地址在节点表反查 router-id（真名）；查不到返回 0。供 VTY 在
 * peer 尚无 remote_id 时将 locator 解析回排除名单使用的稳定身份。 */
extern struct in_addr midr_nds_rid_by_transport(struct midr_g1 *g1,
						struct ipaddr transport);

/* ⑦ `no midr neighbor` 清账：按地址反查节点表条目并 detach 全套（停探+删 link+
 * 清 is_adjacent+拆会话）。查到返回 true，查不到 false（调用方退化为只拆会话）。 */
extern bool midr_nds_detach_by_locator(struct midr_g1 *g1,
				       struct ipaddr addr);
 ...
 */
extern bool midr_nds_is_session_excluded(struct midr_g1 *g1, struct in_addr rid);
extern void midr_nds_session_exclude_add(struct midr_g1 *g1, struct in_addr rid);
extern void midr_nds_session_exclude_del(struct midr_g1 *g1, struct in_addr rid);
/* 名单非空？只给 VTY 判断"要不要为解析不出 rid 而告警"用。 */
extern bool midr_nds_session_blacklist_nonempty(struct midr_g1 *g1);
 ...
 * 对外可见；名字维持不改）。
 */
extern bool midr_discovery_should_peer(struct midr_g1 *g1,
 ...
 */
extern void midr_nds_ledger_note(struct midr_g1 *g1,
				 struct ipaddr transport,
				 enum midr_session_reason reason,
				 struct in_addr remote_rid,
				 as_t remote_asn,
				 uint32_t remote_group);
extern void midr_nds_ledger_drop(struct midr_g1 *g1,
				 struct ipaddr transport);
...
extern void midr_nds_manual_session_set(struct midr_g1 *g1,
					struct ipaddr transport, as_t remote_asn,
					struct in_addr remote_rid);
extern bool midr_nds_manual_session_del(struct midr_g1 *g1,
					struct ipaddr transport);
extern void midr_nds_manual_sessions_restore(struct midr_g1 *g1);

extern bool midr_rep_dir_del(struct midr_g1 *g1, uint32_t group_id,
			     struct ipaddr rep_transport);
extern void midr_rep_dir_clear(struct midr_g1 *g1);
extern struct midr_rep_entry *midr_rep_dir_find_group(struct midr_g1 *g1,
						      uint32_t group_id);
...
extern struct midr_global_view *midr_get_global_view(struct midr_g1 *g1);

/* This node's runtime group-id; 0 if MIDR is not initialised. */
extern uint32_t midr_local_group_id(struct midr_g1 *g1);

/* Look up `node_id` in the node table; on hit, write its group-id to *out and
 * return true, else return false (*out untouched). */
extern bool midr_node_group_id(struct midr_g1 *g1, const struct prefix *node_id,
			       uint32_t *out);

/* Address-family-neutral transport accessors for PM and other consumers. */
extern bool midr_nds_local_transport_get(const struct midr_g1 *g1,
					 struct ipaddr *transport);
/* Set/clear configured intent and apply it through one lifecycle path. */
extern int midr_nds_transport_configure(struct midr_g1 *g1,
					const struct ipaddr *transport);
extern int midr_nds_transport_reconcile(struct midr_g1 *g1);
extern bool midr_nds_node_transport_get(const struct midr_g1 *g1,
					const struct prefix *node_id,
					struct ipaddr *transport);
/* True when no other node identity advertises the same full locator. */
extern bool midr_nds_locator_unique(const struct midr_g1 *g1,
