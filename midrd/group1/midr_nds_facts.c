...
DEFINE_MTYPE_STATIC(MIDR_G1, MIDR_NDS_SNAPSHOT, "MIDR NDS topology snapshot");
...
void midr_nds_facts_init(struct midr_g1 *g1)
{
	struct midr_nds *mi;
	struct midr_nds_facts *f;

	if (!g1 || !g1->midr_nds_info || g1->midr_nds_info->facts)
		return;
...
void midr_nds_facts_finish(struct midr_g1 *g1)
{
	struct midr_nds *mi;
	struct midr_nds_facts *f;
...
static struct midr_nds_facts *facts_of(struct midr_g1 *g1)
{
	if (!g1 || !g1->midr_nds_info)
		return NULL;

	return g1->midr_nds_info->facts;
}
...
bool midr_nds_facts_node_refresh(struct midr_g1 *g1)
{
	struct midr_nds *mi;
	struct midr_nds_facts *f;
	...
	if (midr_nds_local_transport_get(g1, &next.transport_address)) {
	...
	MIDR_LOG("MIDR facts: node 事实更新 rid=%pI4 group=%u caps=0x%" PRIx64,
		 &g1->router_id, next.group_id, next.cap_flags);
	return true;
}
...
struct midr_context *midr_nds_group2_ctx(struct midr_g1 *g1)
{
	struct midr_nds *mi;
 ...
 */
static bool facts_version_exhausted(struct midr_g1 *g1, uint64_t version,
				    const char *why)
{
	struct midr_nds_facts *f = facts_of(g1);
...
int midr_nds_facts_report_after_cfg(struct midr_g1 *g1)
{
	midr_nds_report_node(g1, MIDR_ORIGIN_INIT);
	return 0;
}

int midr_nds_facts_report_after_rid(struct midr_g1 *g1, bool withdraw)
{
	(void)withdraw;
...
/* midrd no longer floods the locator and role bits of a Node, so every
 * accepted Node change is also advertised in group 1's node directory. */
static int facts_node_upsert(struct midr_g1 *g1, struct midr_context *ctx,
			     struct midr_nds_facts *f)
{
	int ret = midr_topology_node_upsert(ctx, &f->node);

	if (!ret)
		midr_nodedir_local_update(g1, &f->node, false);
	return ret;
}

static int facts_node_withdraw(struct midr_g1 *g1, struct midr_context *ctx,
			       struct midr_nds_facts *f)
{
	int ret = midr_topology_node_withdraw(ctx, f->node.node_id,
					      f->node.version);

	if (!ret)
		midr_nodedir_local_update(g1, &f->node, true);
	return ret;
}

void midr_nds_report_node(struct midr_g1 *g1, enum midr_origin_reason reason)
{
	struct midr_nds *mi;
	struct midr_nds_facts *f;
	struct midr_context *ctx;
	...
	changed = midr_nds_facts_node_refresh(g1);
	...
	if (reason == MIDR_ORIGIN_LEAVE || !f->node_valid) {
		...
		if (facts_version_exhausted(g1, f->node.version, why))
			return;

		/* 见下方 upsert 处的 version 说明：撤销同样要带更高的 version，
		 * 否则被对方当旧事件静默丢弃。 */
		f->node.version++;
		ret = facts_node_withdraw(g1, ctx, f);
		if (ret) {
	 ...
	 */
	if (midr_nds_is_bootstrap(g1) || !mi->local_group_id) {
		bool bootstrap = midr_nds_is_bootstrap(g1);
	...
	ret = facts_node_upsert(g1, ctx, f);
	if (ret) {
...
struct midr_nds_fact_link *midr_nds_facts_link_find(struct midr_g1 *g1,
...
struct midr_nds_fact_link *midr_nds_facts_link_get(struct midr_g1 *g1,
						   uint32_t remote_node_id,
						   uint64_t link_id)
{
	struct midr_nds_facts *f = facts_of(g1);
	...
	fl = midr_nds_facts_link_find(g1, remote_node_id, link_id);
	if (fl)
		return fl;

	fl = XCALLOC(MTYPE_MIDR_NDS_FACT_LINK, sizeof(*fl));
	fl->data.key.local_node_id = g1->router_id.s_addr;
...
void midr_nds_facts_link_del(struct midr_g1 *g1, struct midr_nds_fact_link *fl)
{
	struct midr_nds_facts *f = facts_of(g1);
...
uint64_t midr_nds_facts_link_id(struct midr_g1 *g1, uint32_t remote_node_id)
{
	(void)g1;
	(void)remote_node_id;
...
void midr_nds_report_link(struct midr_g1 *g1, const struct midr_link_entry *link)
{
	struct midr_nds *mi;
	struct midr_nds_facts *f;
	struct midr_context *ctx;
	...
	if (!mi->local_group_id) {
		midr_nds_report_link_withdraw(g1, &link->remote_node_id);
	...
	if (!remote_node ||
	    !midr_nds_local_transport_get(g1, &local_locator) ||
	    !midr_nds_node_transport_get(g1, &link->remote_node_id,
					 &remote_locator) ||
	    ipaddr_family(&local_locator) != ipaddr_family(&remote_locator) ||
	    !midr_nds_locator_unique(g1, &link->remote_node_id,
				      &remote_locator)) {
	 ...
	 * 对方预告过"连引导的链路我方主动不上报"。
	 */
	if (midr_nds_is_bootstrap(g1)) {
		midr_nds_report_link_withdraw(g1, &link->remote_node_id);
		MIDR_LOG("MIDR facts: 本机是引导节点，link 上报跳过 (%pFX)",
			 &link->remote_node_id);
		return;
	}
	if (midr_nds_link_is_backbone(g1, &link->remote_node_id)) {
		midr_nds_report_link_withdraw(g1, &link->remote_node_id);
	 ...
	 */
	if (!midr_node_established_peer(g1, &link->remote_node_id)) {
	...
	link_id = midr_nds_facts_link_id(g1, remote_rid);
	fl = midr_nds_facts_link_get(g1, remote_rid, link_id);
	if (!fl)
		return;
	...
	if (facts_version_exhausted(g1, fl->data.version, "link upsert"))
		return;
...
void midr_nds_report_link_withdraw(struct midr_g1 *g1,
				   const struct prefix *remote_node_id)
{
	...
	link_id = midr_nds_facts_link_id(g1, remote_rid);
	fl = midr_nds_facts_link_find(g1, remote_rid, link_id);
	if (!fl)
	...
	if (facts_version_exhausted(g1, fl->data.version, "link withdraw"))
		return;
...
void midr_nds_facts_withdraw_all_links(struct midr_g1 *g1)
{
	struct midr_nds_facts *f = facts_of(g1);
	struct midr_nds_fact_link *fl;
	struct listnode *node;
	...
	for (ALL_LIST_ELEMENTS_RO(f->links, node, fl)) {
		...
		midr_nds_report_link_withdraw(g1, &remote);
	}
}
...
static bool snapshot_node_eligible(struct midr_g1 *g1,
				   const struct midr_nds_facts *f)
{
	...
	if (midr_nds_is_bootstrap(g1) || !g1->midr_nds_info->local_group_id ||
	    g1->midr_nds_info->shutdown)
		return false;

	/* 键合法：他们拿 ctx->bgp->router_id.s_addr 逐位比。 */
	if (!f->node.node_id || f->node.node_id != g1->router_id.s_addr)
		return false;
	if (f->node.group_id != g1->midr_nds_info->local_group_id ||
	    f->node.cap_flags != g1->midr_nds_info->local_capabilities)
		return false;

	/* As with Link endpoints, never replay a Node locator from an older
	 * transport generation. */
	has_active = midr_nds_local_transport_get(g1, &active);
...
static bool snapshot_link_eligible(struct midr_g1 *g1,
				   const struct midr_nds_fact_link *fl)
{
	...
	if (midr_nds_is_bootstrap(g1) || !mi->local_group_id || mi->shutdown ||
	    midr_nds_link_is_backbone(g1, &remote))
		return false;
...
int midr_nds_topology_snapshot_get(struct midr_context *ctx,
				   struct midr_topology_snapshot *snapshot)
{
	...
	if (!g1->router_id.s_addr)
		return -EAGAIN; /* 身份未就绪：键都还没有 */

	/* 一趟数、一趟填 —— 中间不会有人改表（单线程事件循环）。 */
	node_ok = snapshot_node_eligible(g1, f);
	for (ALL_LIST_ELEMENTS_RO(f->links, node, fl))
		if (snapshot_link_eligible(g1, fl))
			link_n++;
...
void midr_nds_topology_snapshot_release(
