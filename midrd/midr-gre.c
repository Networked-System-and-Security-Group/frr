...
/*
 * MIDR GRE virtual interface provisioning (midrd/midr-gre.c)
 *
 * See midr-gre.h for the design overview.  This is the midrd-native port of the
 * former bgpd/bgp_midr_gre.c: the host parameter is struct midr_context and the
 * zclient is the one owned by the MIDR data-plane backend, so a GRE tunnel and
 * the SPF routes share a single ZAPI connection to zebra.
 *
...
/* The GRE module shares the routing backend's zclient and event loop. */
static struct event_loop *midr_gre_master_ptr;

static struct zclient *midr_gre_zclient(void)
{
...
/* The midrd event loop, or NULL when running outside the daemon (tests). */
static struct event_loop *midr_gre_master(void)
{
	return midr_gre_master_ptr;
}

/*
 * Attach the GRE module to the midrd runtime.  Call after the data-plane
 * backend started, so the shared zclient exists.
 */
void midr_gre_init(struct event_loop *master)
{
	midr_gre_master_ptr = master;
}
 ...
 *
 * midrd learns about new netdevices through zebra's ZEBRA_INTERFACE_ADD
...
int midr_gre_interface_add(struct midr_context *ctx,
			   ...
			   struct midr_gre_status *status)
{
	...
	if (!midr_gre_zclient() || midr_gre_zclient()->sock < 0) {
		zlog_warn("%s: zclient not ready", __func__);
		err = ENOTCONN;
		goto out_fail;
	}

	vrf_id = tun->vrf_id == VRF_UNKNOWN ? midr_dp_backend_vrf_id()
					    : tun->vrf_id;
	...
	ret = zclient_send_gre_add(midr_gre_zclient(), vrf_id, &gre);
...
out_fail:
	if (status) {
		...
		if (ctx)
			status->vrf_id = midr_dp_backend_vrf_id();
	}
	return -1;
}

int midr_gre_interface_del(struct midr_context *ctx, const char *ifname,
			   struct midr_gre_status *status)
{
	struct midr_gre_entry *e;
	vrf_id_t vrf_id;
	...
	if (e)
		...
		vrf_id = midr_dp_backend_vrf_id();

	ret = zclient_send_gre_delete(midr_gre_zclient(), vrf_id, ifname);
	if (ret == ZCLIENT_SEND_FAILURE) {
...
int midr_gre_interface_del_by_endpoints(struct midr_context *ctx,
					vrf_id_t vrf_id,
					const struct ipaddr *local,
					const struct ipaddr *remote,
					struct midr_gre_status *status)
{
	...
	return midr_gre_interface_del(ctx, e->ifname, status);
}
...
bool midr_gre_interface_wait_up(struct midr_context *ctx, const char *ifname,
