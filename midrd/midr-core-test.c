...
static struct midr_core_key node_key(uint8_t family)
{
...
static struct midr_core_object active(const struct midr_core_key *key,
					      uint64_t sequence)
{
...
{
	struct midr_core_key key = node_key(MIDR_CORE_AF_IPV4);
...
{
	struct midr_core_key key = node_key(MIDR_CORE_AF_IPV6);
	struct midr_core_object object = active(&key, 1);
	struct midr_core_key noncanonical = key;
	enum midr_core_result result;

	noncanonical.prefix[8] = 0xff;
	assert(midr_core_key_validate(&noncanonical) == -EINVAL);
