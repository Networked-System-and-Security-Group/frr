...
/*
 * MIDR GRE virtual interface provisioning (midr-gre.h)
 *
 * =========================================================================
 *  Purpose
 * =========================================================================
 ...
 *        ▼
 *     midr-gre.c  ──►  zclient_send_gre_add()/delete()  (lib/zclient.c)
 *        │  ZEBRA_GRE_ADD / ZEBRA_GRE_DELETE
 *        ▼
 *     zebra (zapi_msg.c → zebra_dplane.c → if_netlink.c)  ──►  kernel
 *
 *  The request is carried on the MIDR data-plane backend's zclient, so GRE
 *  provisioning and SPF route installation share one ZAPI connection.
 *  midr_gre_init() must therefore be called after the backend has started.
 */
 ...
 *  Establishment state
 * =========================================================================
 * The kernel device is created asynchronously (midrd -> zebra -> netlink).
 ...
 * Pumps the midrd event loop while waiting, so it must be called from the
 * midrd main thread.
 *
...
/*
 * Attach the GRE module to the midrd event loop.  Call once after the
 * data-plane backend has started, so the shared zclient exists.
 */
extern void midr_gre_init(struct event_loop *master);

/*
 * Release the local registry.  Called at midrd shutdown.
 */
