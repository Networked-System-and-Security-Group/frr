...
{
	struct midr_core_key normalized;
	...
	if (key->type == MIDR_CORE_GROUP_PREFIX && !key->group)
		return -EINVAL;
	if (midr_core_key_normalize(key, &normalized))
		return -EINVAL;
...
{
	struct midr_core_key na;
	struct midr_core_key nb;

	if (!a || !b || midr_core_key_normalize(a, &na) ||
	    midr_core_key_normalize(b, &nb))
		return false;
					     ...
					     const struct midr_core_object *b)
{
	if (!a || !b || a->state != b->state ||
	    !midr_core_key_equal(&a->key, &b->key))
		return false;
...
static struct midr_core_entry *find_entry(struct midr_core *core,
					  const struct midr_core_key *key)
{
	for (size_t i = 0; i < core->count; i++)
		if (midr_core_key_equal(&core->entries[i].object.key, key))
			 ...
			 uint64_t now_ms, enum midr_core_result *result)
{
	...
	if (!core || !object || !result || !object->sequence ||
	    ...
	    midr_core_key_normalize(&object->key, &normalized.key) ||
	    midr_core_key_validate(&normalized.key))
		return -EINVAL;
	normalized = *object;
	if (midr_core_key_normalize(&object->key, &normalized.key))
		return -EINVAL;
	...
	entry = find_entry(core, &normalized.key);
	if (entry) {
			       ...
			       const struct midr_core_key *key, uint64_t now_ms)
{
	...
	if (!core || !key || midr_core_key_normalize(key, &normalized) ||
	    midr_core_key_validate(&normalized))
		return -EINVAL;
			       ...
			       size_t *count)
{
	...
	for (size_t i = 0; i < core->count; i++) {
		...
		if (copied == capacity)
			return -ENOSPC;
		if (midr_core_lookup(core, &core->entries[i].object.key, now_ms,
				      &objects[copied], NULL))
			continue;
		copied++;
	}
