#!/bin/bash
# ⚠⚠ 【已作废，勿跑】件②（轮 4）删掉我方自有的 Node NLRI 收发线之后，本脚本验的
#     东西整体不存在了：它靠 z1 自己 originate 一份 Node NLRI、再看 t3 的 BGP-LS 库
#     里那份还在不在；现在 z1 根本不发 (4,8) NLRI，`show bgp link-state link-state`
#     里永远查不到它，脚本必然停在"[准备] 场景没造起来"。
#
#     **它的问题本身没有过期**（"那一发撤销到底出没出去"依旧是记档 39 的核心），
#     只是被测对象换了：撤销现在走 midr_nds_report_node(LEAVE) → 第二组的
#     midr_topology_node_withdraw()，要验就得看远端第二组视图里我方对象消没消失。
#
#     去向（件② plan C-3）：随"退网 5s 延时拆会话"恢复的那一批重写。⚠ 那件事本身
#     卡在第二组的断言崩溃上（记档 56 / 问题清单 #17：`bgp_midr_rib.c:512`
#     assertion (store->active_identity_count) failed，"身份已撤、会话未断"的中间态
#     打破了他们的假设），**等对方修好断言、延时恢复之后再动本脚本**。
#     在那之前记档 39（撤销发不出去）继续挂账。
#
#     以下为原脚本，整份保留备查，不再维护。
# ------------------------------------------------------------------------------
