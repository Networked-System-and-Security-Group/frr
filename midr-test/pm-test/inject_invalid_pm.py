...
def ipv6_udp_packet(source, destination, payload):
    ...
    udp_header = struct.pack("!HHHH", PM_PORT, PM_PORT, udp_length, 0)
    udp_checksum = checksum(pseudo_header + udp_header + payload)
    if udp_checksum == 0:
        udp_checksum = 0xFFFF
    udp_header = struct.pack("!HHHH", PM_PORT, PM_PORT, udp_length,
                             udp_checksum)
...
def send_case(raw_socket, label, source, destination, payload):
    packet = ipv6_udp_packet(source, destination, payload)
    try:
...
def main():
    ...
    cases = [
        ("unknown-source", args.unknown_source,
         probe_payload(seqno=0x10000001)),
        ("bad-magic", args.known_source,
         probe_payload(magic=0xDEADBEEF, seqno=0x10000002)),
        ("unexpected-reply", args.known_source,
         probe_payload(packet_type=1, seqno=0xFFFFFFFF)),
        ("mapped-source", "::ffff:192.0.2.1",
         probe_payload(seqno=0x10000003)),
        ("link-local-source", "fe80::bad",
         probe_payload(seqno=0x10000004)),
        ("multicast-source", "ff02::bad",
         probe_payload(seqno=0x10000005)),
    ]

    for label, source, payload in cases:
