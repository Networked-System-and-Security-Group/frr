...
Usage: sudo -E ./midr-test/pm-test/run_ipv6_test.sh [OPTIONS]
  ...
  --iperf                  Run IPv6 UDP background traffic
...
ip netns exec "$NS_A" tcpdump -U -i "$IF_A" -s 0 -w \
    "$ARTIFACT_DIR/pm-ipv6.pcap" 'udp port 5860' \
...
echo "[run_ipv6_test] creating symmetric IPv6 MIDR overlay sessions"
...
ip netns exec "$NS_A" ss -6 -u -l -n -p >"$ARTIFACT_DIR/ss-node-a.txt"
ip netns exec "$NS_B" ss -6 -u -l -n -p >"$ARTIFACT_DIR/ss-node-b.txt"

if (( IPERF_ENABLE == 1 )); then
    ip netns exec "$NS_B" iperf3 -6 -s -1 -B "$LINK_B" -p 5201 \
        >"$ARTIFACT_DIR/iperf-server.log" 2>&1 &
    IPERF_SERVER_PID=$!
    (
        sleep "$IPERF_START_TIME"
        ip netns exec "$NS_A" iperf3 -6 -c "$LINK_B" -B "$LINK_A" \
