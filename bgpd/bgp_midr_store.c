...
int midr_store_init(void)
{
	...
	if (db_execute("CREATE TABLE IF NOT EXISTS midr_bootstrap_seed ("
		       ...
		       ");") != 0) {
		zlog_warn("midr_store: failed to create midr_bootstrap_seed table");
		return -1;
	}
...
void midr_store_seed_save(const char *transport, uint32_t asn, time_t now)
{
	...
	if (!g_midr_store_initialized || !transport)
		return;

	/* 有则更新、无则插入（SQLite UPSERT，与第二组 node_upsert 无关，纯撞名）。 */
	ss = db_prepare(
		"INSERT INTO midr_bootstrap_seed (transport, asn, last_seen)"
		" VALUES (?, ?, ?)"
		" ON CONFLICT(transport) DO UPDATE SET"
		"   asn = excluded.asn, last_seen = excluded.last_seen;");
	if (!ss)
		return;

	if (db_bindf(ss, "%s%i%d", transport, (int)strlen(transport), asn,
		     (uint64_t)now) != 0) {
		db_finalize(&ss);
		return;
	}
	...
	if (db_run(ss) != SQLITE_OK)
		zlog_warn("midr_store: seed save for %s failed", transport);
...
void midr_store_seed_load(midr_seed_cb cb, void *arg)
{
	...
	ss = db_prepare("SELECT transport, asn FROM midr_bootstrap_seed"
	...
	while (db_run(ss) == SQLITE_ROW) {
		...
		db_loadf(ss, "%s%i", &transport, &asn);
		if (transport)
			cb(transport, asn, arg);
	}
