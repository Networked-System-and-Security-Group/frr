						    ...
						    unsigned int *manual_out)
{
	...
	for (ALL_LIST_ELEMENTS_RO(mi->shutdown_targets, node, target)) {
		if (target->has_sync_generation && bgp->midr_info) {
			...
			if (!midr_ipaddr_to_sockunion(&target->transport, &remote))
				continue;
			...
			if (!peer || !peer->connection ||
				...
				zlog_info("MIDR 退网：跳过已断开或已被新会话替代的目标 %pIA",
					  &target->transport);
				continue;
			}
		}
						 ...
						 const struct ipaddr *transport)
{
	...
	struct ipaddr connected;

	if (!bgp || !transport || !midr_ipaddr_valid_locator(transport))
		return false;
	for (ALL_LIST_ELEMENTS_RO(bgp->peer, node, peer)) {
		connection = peer->connection;
		if (!connection || connection->status != Established ||
		    !peer->afc_nego[AFI_BGP_LS][SAFI_MIDR_LS] ||
		    !midr_sockunion_to_ipaddr(&connection->su, &connected))
			continue;
		if (midr_ipaddr_same(&connected, transport))
			return true;
	}
	return false;
}
...
static void midr_shutdown_targets_snapshot(struct bgp *bgp)
{
	...
	for (ALL_LIST_ELEMENTS_RO(mi->session_ledger, node, entry)) {
		...
		if (bgp->midr_info) {
			...
			if (!midr_ipaddr_to_sockunion(&entry->transport, &remote))
				continue;
			peer = peer_lookup(bgp, &remote);
			if (peer && peer->connection &&
			    midr_sync_connection_generation(
