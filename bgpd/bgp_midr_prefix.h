...
enum midr_prefix_local_source {
	MIDR_PREFIX_SOURCE_NETWORK = (1U << 0),
	MIDR_PREFIX_SOURCE_CONNECTED = (1U << 1),
	MIDR_PREFIX_SOURCE_STATIC = (1U << 2),
};
...
extern int midr_prefix_local_source_set(struct midr_context *ctx, afi_t afi, uint32_t source,
					bool enabled);
extern uint32_t midr_prefix_local_sources(struct midr_context *ctx, afi_t afi);
extern int midr_prefix_external_peer_set(struct midr_context *ctx, struct peer *peer, afi_t afi,
					 bool enabled);
