...
C="frr-ubuntu24-ymy"
DS="echo thu325325 | sudo -S docker"

log_info "Step 1: Fix file ownership..."
eval "$DS exec -u 0 $C bash -c 'chown -R frr:frr /home/frr/frr/bgpd/ 2>/dev/null || true'" 2>/dev/null

log_info "Step 2: Compile bgpd..."
COMPILE_OUT=$(eval "$DS exec -u 0 $C bash -c '
cd /home/frr/frr
...
BIN_SIZE=$(eval "$DS exec $C ls -la /home/frr/frr/bgpd/.libs/bgpd 2>/dev/null" | awk '{print $5}')
[ -n "$BIN_SIZE" ] && log_pass "bgpd binary exists (${BIN_SIZE} bytes)" || log_fail "bgpd binary missing"

OBJ_SIZE=$(eval "$DS exec $C ls -la /home/frr/frr/bgpd/bgp_midr_zebra.o 2>/dev/null" | awk '{print $5}')
...
SYMBOLS=$(eval "$DS exec $C bash -c 'cd /home/frr/frr && nm bgpd/bgp_midr_zebra.o | grep \" T \"'" 2>&1)
echo "$SYMBOLS"
...
eval "$DS exec $C bash -c 'cd /home/frr/frr && ar t bgpd/libbgp.a'" 2>/dev/null | grep -q "bgp_midr_zebra" && log_pass "bgp_midr_zebra.o in libbgp.a" || log_fail "bgp_midr_zebra.o NOT in libbgp.a"

echo ""
echo "============================================"
echo "  Test 01 Summary: PASS=$PASS FAIL=$FAIL"
echo "============================================"
[ $FAIL -eq 0 ] && echo "  BUILD VERIFICATION PASSED" || echo "  BUILD VERIFICATION FAILED"
echo "============================================"
exit $FAIL