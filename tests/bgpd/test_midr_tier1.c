...
/* Test fixture only: production membership comes exclusively from a file. */
static const as_t test_asns[] = {174, 1299, 3356};
static const struct midr_tier1_list test_tier1_list = {
	.asns = test_asns,
	.count = array_size(test_asns),
	.version = "unit-test-policy",
};
...
static void test_ordered_tier1_hit(void)
{
	...
	ret = midr_tier1_aspath_check(path, &test_tier1_list,
				      &result);
...
static void test_invalid_arguments(void)
{
	...
	expect_true(midr_tier1_aspath_check(path, &test_tier1_list,
					    NULL) == -1,
